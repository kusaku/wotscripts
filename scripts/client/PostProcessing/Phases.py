...
@implementPhaseFactory('FXAA', 'Do FXAA on the screen.', None, None)
def buildFXAAPhase(input, output):
    p = buildPhase(input, output, 'shaders/post_processing/fxaa.fx', straightTransfer4Tap, BW_BLEND_ONE, BW_BLEND_ZERO)
    p.name = 'fxaa'
    return p
