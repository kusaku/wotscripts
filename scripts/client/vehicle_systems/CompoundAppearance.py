...
class CompoundAppearance(ComponentSystem, CallbackDelayer):
    ...
    def __onModelsRefresh(self, modelState, resourceList):
        if not self.damageState.isCurrentModelDamaged:
            ...
            return (BattleReplay.isFinished() or self.hidden) and None
        else:
            if not modelState == self.__currentDamageState.modelState:
                ...
                if self.__currentDamageState.isCurrentModelDamaged:
                    ...
                    self.swingingAnimator = None
                    self.gunRecoil = None
                    self.additionalTurretsRotator is not None and self.additionalTurretsRotator.destroy()
    ...
    def getTurretMatrix(self, turretIndex):
        if turretIndex < len(self.turretMatrixList):
            return self.turretMatrixList[turretIndex]
        else:
            return None
