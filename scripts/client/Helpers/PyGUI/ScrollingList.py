...
class ScrollingList(PyGUIBase):
    ...
    def createItem(self):
        raise self.itemGuiDS is not None or AssertionError
    ...
    def handleKeyEvent(self, event):
        ...
        if event.isKeyDown():
            if self.handleTraversalKeys(event):
                return True
            if key == [Keys.KEY_JOYA] or key in [Keys.KEY_RETURN, Keys.KEY_NUMPADENTER] and not BigWorld.isKeyDown(Keys.KEY_LALT, 0) and not BigWorld.isKeyDown(Keys.KEY_RALT, 0):
                if len(self.items.children) == 0:
                    return True
