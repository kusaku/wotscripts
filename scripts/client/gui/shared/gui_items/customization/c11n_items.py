...
class Customization(FittingItem):
    ...
    def mayInstall(self, vehicle, _ = None):
        ...
        """
        if not self.descriptor.filter:
            return True
        return self.descriptor.filter.matchVehicleType(vehicle.descriptor.type)
...
class Style(Customization):
    ...
    def userType(self):
        ...
        """
        if self.isRentable:
            return i18n.makeString(VEHICLE_CUSTOMIZATION.CAROUSEL_SWATCH_STYLE_RENT)
        return i18n.makeString(VEHICLE_CUSTOMIZATION.CAROUSEL_SWATCH_STYLE_PERMANENT)
