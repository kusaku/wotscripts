...
class EventBattleSquadEntity(SquadEntity):
    ...
    def canChangeBattleType(self):
        ...
        if not self.isCommander():
            return False
        if self._cooldown.isInProcess(REQUEST_TYPE.CHANGE_EVENT_QUEUE_TYPE):
            return False
        return True
    ...
    def _vehicleStateCondition(self, vehicle):
        if VEHICLE_TAGS.EVENT not in vehicle.tags:
            return False
        return super(EventBattleSquadEntity, self)._vehicleStateCondition(vehicle)