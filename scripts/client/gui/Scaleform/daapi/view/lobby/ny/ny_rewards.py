...
class NYRewards(NYHelperView, NYScreenRewardsMeta):
    ...
    def __getStateByLevel(self, level):
        ...
        if level <= self.__nyMaxLevel:
            return NY_CONSTANTS.REWARDS_LEVEL_RECEIVED
        return NY_CONSTANTS.REWARDS_LEVEL_NEXT

    def __getVehicleDiscountStatus(self, hasChests, hasVehicleDiscounts):
        if hasChests:
            return NY_CONSTANTS.NY_VEHICLE_DISCOUNT_AVAILABLE
        if hasVehicleDiscounts:
            return NY_CONSTANTS.NY_VEHICLE_DISCOUNT_RECEIVED
        return NY_CONSTANTS.NY_VEHICLE_DISCOUNT_APPLIED
