...
class NYScreenViewBase(NYScreenViewMeta):
    ...
    def __setSlotsPosition(self):
        ...
        if not anchorName is not None:
            ...
            raise leftPointScreenX is not None or AssertionError
            if not rightPointScreenX is not None:
                raise AssertionError
                leftPointScreenX = self.__interfaceScale > 1.0 and round(leftPointScreenX / self.__interfaceScale)
