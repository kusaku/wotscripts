...
def toyTypeByIndex(index):
    if index == 0 or len(TOY_TYPES) < index:
        return None
    else:
        return TOY_TYPES[index - 1]
...
def toyNationByIndex(index):
    ...
    elif index == 0 or len(NATIONAL_SETTINGS) < index:
        return None
    else:
        return NATIONAL_SETTINGS[index - 1]
...
def indexByToyType(type):
    if type is None:
        ...
        return TOY_TYPES_IDS_BY_NAME.get(type, None) + 1
...
def indexByToyNation(nation):
    if nation == 'modernWestern':
        ...
        return NATIONAL_SETTINGS_IDS_BY_NAME.get(nation, None) + 1


def getStyle(isEnabled):
    if isEnabled:
        return text_styles.tutorial
