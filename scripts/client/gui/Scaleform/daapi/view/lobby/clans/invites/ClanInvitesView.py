...
class InviteDataProvider(ClanInvitesAbstractDataProvider):
    ...
    def _makeVO(self, item, extra):
        return {'dbID': item.getDbID(),
         ...
         'awgExp': formatField(getter=item.getBattleXpAvg, formatter=lambda value: BigWorld.wg_getNiceNumberFormat(value) + '%'),
         'wins': formatField(getter=item.getBattlesPerformanceAvg, formatter=BigWorld.wg_getIntegralFormat),
