...
class QuestsSeasonAwardsWindow(QuestsSeasonAwardsWindowMeta):
    ...
    def _populate(self):
        ...
        self.addListener(EventsWindowContentTabsEvent.TAB_SELECTED, self.__onTabSelected)
        self.__setData()
    ...
    def __setData(self):
        awards = []
        cache = events_helpers.getPotapovQuestsCache(self.__questsType)
    ...
    def __packBasicAward(self, tile):
        ...
        tokensCountText = text_styles.main(_ms(QUESTS.SEASONAWARDSWINDOW_TOKENSCOUNT, count=str(tokensCount), icon=tokenIcon))
        return {'iconPath': '../maps/icons/quests/awards/tile_%s_%s_award.png' % (tile.getSeasonID(), tile.getID()),
         ...
         'tokensCountTooltip': TOOLTIPS_CONSTANTS.PRIVATE_QUESTS_TOKENS_AWARD}
