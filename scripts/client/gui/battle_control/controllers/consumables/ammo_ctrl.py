...
class AmmoController(MethodsRules, IBattleController):
    ...
    def getSubGunsCount(self):
        if len(self.__gunSettings) > 1:
            return len(self.__gunSettings[1:])
        return 0
    ...
    def getCurrentShellCD(self, turretIndex):
        ...
        elif len(self.__currSubShellCD) >= turretIndex:
            return self.__currSubShellCD[turretIndex]
        else:
            return None
    ...
    def getCurrentShells(self, turretIndex):
        ...
        if shellCD is not None:
            return self.getShells(shellCD, turretIndex)
        else:
