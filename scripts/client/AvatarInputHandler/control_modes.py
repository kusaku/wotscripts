...
class PostMortemControlMode(IControlMode):
    ...
    def enable(self, **args):
        ...
        respawnCtrl = bool(args.get('respawn', False)) and self.guiSessionProvider.dynamic.respawn
        if not respawnCtrl is not None:
            raise AssertionError('When using respawn, the respawn controller must exist')
    ...
    def altTargetMode(self, mode):
        raise mode in CTRL_MODES or mode is None or AssertionError('Invalid control mode set as alternative target control mode.')
