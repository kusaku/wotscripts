# Embedded file name: scripts/common/pydev/_pydev_bundle/pydev_monkey.py
import os
import sys
import traceback
try:
...
def _get_python_c_args(host, port, indC, args):
    return "import sys; sys.path.append(r'%s'); import pydevd; pydevd.settrace(host='%s', port=%s, suspend=False, trace_only_current_thread=False, patch_multiprocessing=True); %s" % (pydev_src_dir,
     host,
     port,
     args[indC + 1])
...
def patch_args(args):
    try:
        ...
        if i < len(args) and _is_managed_arg(args[i]):
            return args
        for x in original:
...
def warn_multiproc():
    log_error_once("pydev debugger: New process is launching (breakpoints won't work in the new process).\npydev debugger: To debug that process please enable 'Attach to subprocess automatically while debugging?' option in the debugger settings.\n")
