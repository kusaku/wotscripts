# Embedded file name: scripts/common/pydev/_pydevd_bundle/pydevd_process_net_command.py
import os
import sys
import traceback
...
def process_net_command(py_db, cmd_id, seq, text):
    ...
    try:
        ...
        if cmd_id == CMD_RUN:
        ...
        elif cmd_id == CMD_STEP_INTO or cmd_id == CMD_STEP_OVER or cmd_id == CMD_STEP_RETURN or cmd_id == CMD_STEP_INTO_MY_CODE:
            t = pydevd_find_thread_by_id(text)
            if t:
                thread_id = get_thread_id(t)
            ...
            if not pydevd_file_utils.exists(file):
                sys.stderr.write('pydev debugger: warning: trying to add breakpoint to file that does not exist: %s (will have no effect)\n' % (file,))
                sys.stderr.flush()
            ...
            int_cmd = InternalEvaluateExpression(seq, thread_id, frame_id, expression, cmd_id == CMD_EXEC_EXPRESSION, int(trim) == 1)
            py_db.post_internal_command(int_cmd, thread_id)
            ...
            if len(splitted) >= 4:
                ...
                for exception_type in splitted[4:]:
                    ...
                    if not exception_type:
                        continue
                    exception_breakpoint = py_db.add_break_on_exception(exception_type, notify_always=break_on_caught, notify_on_terminate=break_on_uncaught, notify_on_first_raise_only=False)
            ...
            if text.find('\t') != -1:
                ...
                exception, notify_always, notify_on_terminate, ignore_libraries = (text,
                 0,
                 0,
                 0)
            if exception.find('-') != -1:
            ...
            if breakpoint_type == 'python':
                ...
                exception_breakpoint = py_db.add_break_on_exception(exception, notify_always=int(notify_always) > 0, notify_on_terminate=int(notify_on_terminate) == 1, notify_on_first_raise_only=int(notify_always) == 2, ignore_libraries=int(ignore_libraries) > 0)
