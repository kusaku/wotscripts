# Embedded file name: scripts/common/pydev/_pydevd_bundle/pydevd_signature.py
import inspect
try:
    import trace
except ImportError:
    ...
    def file_module_function_of(self, frame):
        ...
        if code in self._caller_cache:
            ...
            if len(funcs) == 1:
                ...
                if len(dicts) == 1:
                    classes = [ c for c in gc.get_referrers(dicts[0]) if hasattr(c, '__bases__') or inspect.isclass(c) ]
                elif len(dicts) > 1:
                    classes = [ c for c in gc.get_referrers(dicts[1]) if hasattr(c, '__bases__') or inspect.isclass(c) ]
                else:
                    classes = []
                if len(classes) == 1:
