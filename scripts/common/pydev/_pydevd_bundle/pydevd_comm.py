# Embedded file name: scripts/common/pydev/_pydevd_bundle/pydevd_comm.py
...
GetGlobalDebugger = get_global_debugger
...
class PyDBDaemonThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            if IS_JYTHON:
                import org.python.core as PyCore
...
class WriterThread(PyDBDaemonThread):
    ...
    def _on_run(self):
        ...
        self._stop_trace()
        get_has_timeout = sys.hexversion >= 33751040
        try:
            while True:
                try:
                    try:
...
class NetCommandFactory:
        ...
        try:
            while curr_frame:
                ...
                if curr_frame.f_code is None:
                    break
                my_name = curr_frame.f_code.co_name
                if my_name is None:
                    break
    ...
    def make_send_curr_exception_trace_message(self, seq, thread_id, curr_frame_id, exc_type, exc_desc, trace_obj):
        try:
            ...
            payload = str(curr_frame_id) + '\t' + exc_type + '\t' + exc_desc + '\t' + self.make_thread_suspend_str(thread_id, trace_obj.tb_frame, CMD_SEND_CURR_EXCEPTION_TRACE, '')
    ...
    def do_it(self, dbg):
        ...
        try:
            ...
            if hasattr(keys, 'sort'):
                keys.sort(compare_object_attrs)
            elif IS_PY3K:
                keys = sorted(keys, key=cmp_to_key(compare_object_attrs))
            else:
                keys = sorted(keys, cmp=compare_object_attrs)
            for k in keys:
    ...
    def do_it(self, dbg):
        try:
            ...
            for filename, line, methodname, methodobj in self.stacktrace:
                ...
                callstack += '<frame thread_id = "%s" file="%s" line="%s" name="%s" obj="%s" />' % (self.thread_id,
                 makeValid(filename),
                 line,
...
class InternalRunCustomOperation(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            res = pydevd_vars.custom_operation(self.thread_id, self.frame_id, self.scope, self.attrs, self.style, self.code_or_file, self.fnname)
