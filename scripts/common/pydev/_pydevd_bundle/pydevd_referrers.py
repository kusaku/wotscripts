# Embedded file name: scripts/common/pydev/_pydevd_bundle/pydevd_referrers.py
    ...
    try:
        try:
            if searched_obj is None:
                ret = ['<xml>\n']
                ret.append('<for>\n')
                ret.append(pydevd_vars.var_to_xml(searched_obj, 'Skipping getting referrers for None', additionalInXml=' id="%s"' % (id(searched_obj),)))
                ret.append('</for>\n')
                ret.append('</xml>')
                ret = ''.join(ret)
                return ret
            ...
            try:
                ...
                ret = ['<xml>\n']
                ret.append('<for>\n')
                ret.append(pydevd_vars.var_to_xml(searched_obj, 'Exception raised while trying to get_referrers.', additionalInXml=' id="%s"' % (id(searched_obj),)))
                ret.append('</for>\n')
                ret.append('</xml>')
                ret = ''.join(ret)
                return ret
            ...
            ret.append(pydevd_vars.var_to_xml(searched_obj, 'Referrers of obj with id="%s"' % (obj_id,)))
            ret.append('</for>\n')
            all_objects = None
            for r in referrers:
                try:
                ...
                ret.append(pydevd_vars.var_to_xml(r, representation, additionalInXml=' id="%s"%s' % (r_id, found_as)))
        ...
        ret = ['<xml>\n']
        ret.append('<for>\n')
        ret.append(pydevd_vars.var_to_xml(searched_obj, 'Error getting referrers for:', additionalInXml=' id="%s"' % (id(searched_obj),)))
        ret.append('</for>\n')
        ret.append('</xml>')
        ret = ''.join(ret)
        return ret
