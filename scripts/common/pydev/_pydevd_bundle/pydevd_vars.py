# Embedded file name: scripts/common/pydev/_pydevd_bundle/pydevd_vars.py
...
addAdditionalFrameById = add_additional_frame_by_id
...
def array_to_meta_xml(array, name, format):
    ...
    xml = '<array slice="%s" rows="%s" cols="%s" format="%s" type="%s" max="%s" min="%s"/>' % (slice,
     rows,
     cols,
     format,
     type,
