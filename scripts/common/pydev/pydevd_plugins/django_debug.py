# Embedded file name: scripts/common/pydev/pydevd_plugins/django_debug.py
...
def cmd_step_into(plugin, main_debugger, frame, event, args, stop_info, stop):
    ...
    if _is_django_suspended(thread):
        ...
        if stop:
            info.pydev_django_resolve_frame = True
...
def exception_break(plugin, main_debugger, pydb_frame, frame, args, arg):
    ...
    if main_debugger.django_exception_break and get_exception_name(exception) in ('VariableDoesNotExist', 'TemplateDoesNotExist', 'TemplateSyntaxError') and just_raised(trace) and _is_django_exception_break_context(frame):
