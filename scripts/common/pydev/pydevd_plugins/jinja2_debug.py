# Embedded file name: scripts/common/pydev/pydevd_plugins/jinja2_debug.py
import traceback
...
def _is_jinja2_internal_function(frame):
    return dict_contains(frame.f_locals, 'self') and frame.f_locals['self'].__class__.__name__ in ('LoopContext', 'TemplateReference', 'Macro', 'BlockReference')
...
def _find_render_function_frame(frame):
    old_frame = frame
    try:
        while not (dict_contains(frame.f_locals, 'self') and frame.f_locals['self'].__class__.__name__ == 'Template' and frame.f_code.co_name == 'render'):
            frame = frame.f_back
            if frame is None:
                return old_frame
...
def cmd_step_over(plugin, pydb, frame, event, args, stop_info, stop):
    ...
    if _is_jinja2_suspended(thread):
        ...
        return (stop, plugin_stop)
    elif event == 'return' and _is_jinja2_context_call(frame.f_back):
        info.pydev_call_from_jinja2 = None
...
def get_breakpoint(plugin, pydb, pydb_frame, frame, event, args):
    ...
    flag = False
    type = 'jinja2'
    if event in ('line', 'call') and info.pydev_state != STATE_SUSPEND and pydb.jinja2_breakpoints and _is_jinja2_render_call(frame):
        filename = _get_jinja2_template_filename(frame)
        jinja2_breakpoints_for_file = pydb.jinja2_breakpoints.get(filename)
