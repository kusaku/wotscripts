# Embedded file name: scripts/common/pydev/pydevd_concurrency_analyser/pydevd_concurrency_logger.py
import time
...
def log_new_thread(global_debugger):
    ...
    send_message('threading_event', event_time, t.getName(), get_thread_id(t), 'thread', 'start', 'code_name', 0, None, parent=get_thread_id(t))
    return
            ...
            if write_log:
                ...
                if isinstance(self_obj, threading.Thread):
                    ...
                    if method_name in THREAD_METHODS and (back_base not in DONT_TRACE_THREADING or method_name in INNER_METHODS and back_base in INNER_FILES):
                        ...
                        if real_method == 'start':
                            parent = get_thread_id(t)
                        send_message('threading_event', event_time, name, thread_id, 'thread', real_method, back.f_code.co_filename, back.f_lineno, back, parent=parent)
                        ...
                        if dict_contains(back.f_locals, 'self'):
                            if isinstance(back.f_locals['self'], threading.Thread):
                                ...
                                if IS_PY3K and hasattr(my_self_obj, '_pydev_join_called'):
                                    send_massage = False
                                if send_massage:
                                    send_message('threading_event', event_time, 'Thread', my_thread_id, 'thread', 'stop', my_back.f_code.co_filename, my_back.f_lineno, my_back, parent=None)
                if self_obj.__class__ == ObjectWrapper:
                    ...
                    if method_name == '__init__':
                        send_message('threading_event', event_time, t.getName(), get_thread_id(t), 'lock', method_name, back.f_code.co_filename, back.f_lineno, back, lock_id=str(id(frame.f_locals['self'])))
                    if dict_contains(frame.f_locals, 'attr') and (frame.f_locals['attr'] in LOCK_METHODS or frame.f_locals['attr'] in QUEUE_METHODS):
                        ...
                        if real_method == 'release_end':
                            return
                        send_message('threading_event', event_time, t.getName(), get_thread_id(t), 'lock', real_method, back.f_code.co_filename, back.f_lineno, back, lock_id=str(id(self_obj)))
                        if real_method in ('put_end', 'get_end'):
                            send_message('threading_event', event_time, t.getName(), get_thread_id(t), 'lock', 'release', back.f_code.co_filename, back.f_lineno, back, lock_id=str(id(self_obj)))
        except Exception:
            traceback.print_exc()
            ...
            if dict_contains(frame.f_locals, 'self'):
                ...
                if isinstance(self_obj, asyncio.Task):
                    ...
                    if method_name == 'set_result':
                        ...
                        send_message('asyncio_event', event_time, task_name, task_name, 'thread', 'stop', frame.f_code.co_filename, frame.f_lineno, frame)
                    method_name = back.f_code.co_name
                    if method_name == '__init__':
                        task_id = id(self_obj)
                        task_name = self.task_mgr.get(str(task_id))
                        send_message('asyncio_event', event_time, task_name, task_name, 'thread', 'start', frame.f_code.co_filename, frame.f_lineno, frame)
                method_name = frame.f_code.co_name
                    ...
                    if method_name in ('acquire', 'release'):
                        ...
                        if method_name == 'acquire':
                            if not self_obj._waiters and not self_obj.locked():
                                send_message('asyncio_event', event_time, task_name, task_name, 'lock', method_name + '_begin', frame.f_code.co_filename, frame.f_lineno, frame, lock_id=str(id(self_obj)))
                            if self_obj.locked():
                                method_name += '_begin'
                            else:
                                method_name += '_end'
                            ...
                            method_name += '_end'
                        send_message('asyncio_event', event_time, task_name, task_name, 'lock', method_name, frame.f_code.co_filename, frame.f_lineno, frame, lock_id=str(id(self_obj)))
                if isinstance(self_obj, asyncio.Queue):
                    if method_name in ('put', 'get', '_put', '_get'):
                        task_id = self.get_task_id(frame)
                        task_name = self.task_mgr.get(str(task_id))
                        if method_name == 'put':
                            send_message('asyncio_event', event_time, task_name, task_name, 'lock', 'acquire_begin', frame.f_code.co_filename, frame.f_lineno, frame, lock_id=str(id(self_obj)))
                        elif method_name == '_put':
                            send_message('asyncio_event', event_time, task_name, task_name, 'lock', 'acquire_end', frame.f_code.co_filename, frame.f_lineno, frame, lock_id=str(id(self_obj)))
                            send_message('asyncio_event', event_time, task_name, task_name, 'lock', 'release', frame.f_code.co_filename, frame.f_lineno, frame, lock_id=str(id(self_obj)))
