# Embedded file name: scripts/common/pydev/pydevd.py
"""
...
from _pydevd_bundle.pydevd_comm import CMD_SET_BREAK, CMD_SET_NEXT_STATEMENT, CMD_STEP_INTO, CMD_STEP_OVER, CMD_STEP_RETURN, CMD_STEP_INTO_MY_CODE, CMD_THREAD_SUSPEND, CMD_RUN_TO_LINE, CMD_ADD_EXCEPTION_BREAK, CMD_SMART_STEP_INTO, InternalConsoleExec, NetCommandFactory, PyDBDaemonThread, _queue, ReaderThread, GetGlobalDebugger, get_global_debugger, set_global_debugger, WriterThread, pydevd_find_thread_by_id, pydevd_log, start_client, start_server, InternalGetBreakpointException, InternalSendCurrExceptionTrace, InternalSendCurrExceptionTraceProceeded
from _pydevd_bundle.pydevd_constants import IS_JYTH_LESS25, IS_PY3K, IS_PY34_OLDER, get_thread_id, dict_keys, dict_pop, dict_contains, dict_iter_items, DebugInfoHolder, PYTHON_SUSPEND, STATE_SUSPEND, STATE_RUN, get_frame, xrange, clear_cached_thread_id
...
class CheckOutputThread(PyDBDaemonThread):
    ...
    def _on_run(self):
        if self.dontTraceMe:
            ...
            if pydevd_vm_type.get_vm_type() == pydevd_vm_type.PydevdVmType.JYTHON and sys.hexversion <= 33686000:
                disable_tracing = False
            if disable_tracing:
                pydevd_tracing.SetTrace(None)
        while not self.killReceived:
            time.sleep(0.3)
            if not self.py_db.has_threads_alive() and self.py_db.writer.empty() and not has_data_to_redirect():
                try:
...
class PyDB():
    ...
    def get_internal_queue(self, thread_id):
        ...
        try:
            ...
            return self._cmd_queue.setdefault(thread_id, _queue.Queue())
    ...
    def check_output_redirect(self):
        ...
        if bufferStdOutToServer:
            ...
            self.check_output(sys.stdoutBuf, 1)
        if bufferStdErrToServer:
            init_stderr_redirect()
            self.check_output(sys.stderrBuf, 2)
    ...
    def add_break_on_exception(self, exception, notify_always, notify_on_terminate, notify_on_first_raise_only, ignore_libraries = False):
        try:
            eb = ExceptionBreakpoint(exception, notify_always, notify_on_terminate, notify_on_first_raise_only, ignore_libraries)
        except ImportError:
...
def settrace(host = None, stdoutToServer = False, stderrToServer = False, port = 5678, suspend = True, trace_only_current_thread = False, overwrite_prev_trace = False, patch_multiprocessing = False):
    ...
    try:
        _locked_settrace(host, stdoutToServer, stderrToServer, port, suspend, trace_only_current_thread, overwrite_prev_trace, patch_multiprocessing)
    finally:
...
def _locked_settrace(host, stdoutToServer, stderrToServer, port, suspend, trace_only_current_thread, overwrite_prev_trace, patch_multiprocessing):
    global bufferStdOutToServer
    global connected
    global bufferStdErrToServer
...
def stoptrace():
    ...
    if connected:
        ...
        debugger = get_global_debugger()
        if debugger:
            debugger.set_trace_for_frame_and_parents(get_frame(), also_add_to_passed_frame=True, overwrite_prev_trace=True, dispatch_func=lambda *args: None)
            debugger.exiting()
            kill_all_pydev_threads()
        connected = False
    return
