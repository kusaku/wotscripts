# Embedded file name: scripts/common/Lib/test/test_sysconfig.py
"""Tests for sysconfig."""
import unittest
import sys
import os
...
import sysconfig
from sysconfig import get_paths, get_platform, get_config_vars, get_path, get_path_names, _INSTALL_SCHEMES, _get_default_scheme, _expand_vars, get_scheme_names, get_config_var
...
class TestSysConfig(unittest.TestCase):
    ...
    def test_get_platform(self):
        ...
        try:
            ...
            sys.maxint = 9223372036854775807L
    ...
    def test_get_scheme_names(self):
        wanted = ('nt', 'nt_user', 'os2', 'os2_home', 'osx_framework_user', 'posix_home', 'posix_prefix', 'posix_user')
    ...
    def test_symlink(self):
        ...
        def get(python):
            cmd = [python, '-c', 'import sysconfig; print sysconfig.get_platform()']
    ...
    def test_platform_in_subprocess(self):
        ...
        with open('/dev/null', 'w') as devnull_fp:
            p = subprocess.Popen([sys.executable, '-c', 'import sysconfig; print(sysconfig.get_platform())'], stdout=subprocess.PIPE, stderr=devnull_fp, env=env)
        ...
        p = subprocess.Popen([sys.executable, '-c', 'import sysconfig; print(sysconfig.get_platform())'], stdout=subprocess.PIPE, stderr=open('/dev/null'), env=env)
        test_platform = p.communicate()[0].strip()
        test_platform = test_platform.decode('utf-8')
        status = p.wait()
        self.assertEqual(status, 0)
