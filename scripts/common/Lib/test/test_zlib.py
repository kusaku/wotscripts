# Embedded file name: scripts/common/Lib/test/test_zlib.py
import unittest
...
class ChecksumTestCase(unittest.TestCase):
    ...
    def test_crc32start(self):
        ...
        self.assertTrue(zlib.crc32('abc', 4294967295L))
    ...
    def test_adler32start(self):
        ...
        self.assertTrue(zlib.adler32('abc', 4294967295L))
    ...
    def assertEqual32(self, seen, expected):
        self.assertEqual(seen & 4294967295L, expected & 4294967295L)

    def test_penguins(self):
        self.assertEqual32(zlib.crc32('penguin', 0), 3854672160L)
        self.assertEqual32(zlib.crc32('penguin', 1), 1136044692)
        self.assertEqual32(zlib.adler32('penguin', 0), 198116086)
        self.assertEqual32(zlib.adler32('penguin', 1), 198574839)
    ...
    def test_negative_crc_iv_input(self):
        self.assertEqual(zlib.crc32('ham', -1), zlib.crc32('ham', 4294967295L))
        self.assertEqual(zlib.crc32('spam', -3141593), zlib.crc32('spam', 4291825703L))
        self.assertEqual(zlib.crc32('spam', -2147483648L), zlib.crc32('spam', 2147483648L))
...
class BaseCompressTestCase(object):
    ...
    def check_big_compress_buffer(self, size, compress_func):
        _1M = 1048576
        fmt = '%%0%dx' % (2 * _1M)
        data = ''.join([ binascii.a2b_hex(fmt % random.getrandbits(8 * _1M)) for i in range(10) ])
...
class CompressTestCase(BaseCompressTestCase, unittest.TestCase):
    ...
    def test_incomplete_stream(self):
        x = zlib.compress(HAMLET_SCENE)
        self.assertRaisesRegexp(zlib.error, 'Error -5 while decompressing data: incomplete or truncated stream', zlib.decompress, x[:-1])

    @precisionbigmemtest(size=_1G + 1048576, memuse=3)
    def test_big_compress_buffer(self, size):
        compress = lambda s: zlib.compress(s, 1)
        self.check_big_compress_buffer(size, compress)

    @precisionbigmemtest(size=_1G + 1048576, memuse=2)
...
class CompressObjectTestCase(BaseCompressTestCase, unittest.TestCase):
    ...
    def test_clear_unconsumed_tail(self):
        cdata = 'x\x9cKLJ\x06\x00\x02M\x01'
        dco = zlib.decompressobj()
    ...
    def test_flushes(self):
        ...
        for sync in sync_opt:
            for level in range(10):
                ...
                d = obj.flush()
                self.assertEqual(zlib.decompress(''.join([a,
                 b,
                 c,
                 d])), data, 'Decompress failed: flush mode=%i, level=%i' % (sync, level))
                del obj
    ...
    def test_odd_flush(self):
        ...
        if hasattr(zlib, 'Z_SYNC_FLUSH'):
            ...
            gen.seed(1)
            data = genblock(1, 17408, generator=gen)
...
def test_main():
    run_unittest(ChecksumTestCase, ExceptionTestCase, CompressTestCase, CompressObjectTestCase)
