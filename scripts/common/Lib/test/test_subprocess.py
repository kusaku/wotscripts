# Embedded file name: scripts/common/Lib/test/test_subprocess.py
import unittest
...
class ProcessTestCase(BaseTestCase):
    ...
    def test_stdout_none(self):
        p = subprocess.Popen([sys.executable, '-c', 'print "    this bit of output is from a test of stdout in a different process ..."'], stdin=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def test_executable_with_cwd(self):
        ...
        p = subprocess.Popen(['somethingyoudonthave', '-c', 'import sys; sys.exit(47)'], executable=sys.executable, cwd=python_dir)
        p.wait()
        self.assertEqual(p.returncode, 47)

    @unittest.skipIf(sysconfig.is_python_build(), 'need an installed Python. See #7774')
    def test_executable_without_cwd(self):
        p = subprocess.Popen(['somethingyoudonthave', '-c', 'import sys; sys.exit(47)'], executable=sys.executable)
        p.wait()
        self.assertEqual(p.returncode, 47)
    ...
    def test_stdout_filedes_of_stdout(self):
        cmd = "import sys, os; sys.exit(os.write(sys.stdout.fileno(), '.\\n'))"
    ...
    def test_env(self):
        newenv = os.environ.copy()
        newenv['FRUIT'] = 'orange'
        p = subprocess.Popen([sys.executable, '-c', 'import sys,os;sys.stdout.write(os.getenv("FRUIT"))'], stdout=subprocess.PIPE, env=newenv)
        self.addCleanup(p.stdout.close)
        self.assertEqual(p.stdout.read(), 'orange')
    ...
    def test_no_leaking(self):
        ...
        try:
            for i in range(max_handles):
                try:
                    handles.append(os.open(test_support.TESTFN, os.O_WRONLY | os.O_CREAT))
                except OSError as e:
    ...
    def test_leaking_fds_on_error(self):
        for i in range(1024):
            with self.assertRaises(EnvironmentError) as c:
                subprocess.Popen(['nonexisting_i_hope'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def test_communicate_epipe(self):
        p = subprocess.Popen([sys.executable, '-c', 'pass'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        self.addCleanup(p.stdout.close)
        self.addCleanup(p.stderr.close)
        self.addCleanup(p.stdin.close)
        p.communicate('x' * 1048576)

    def test_communicate_epipe_only_stdin(self):
        p = subprocess.Popen([sys.executable, '-c', 'pass'], stdin=subprocess.PIPE)
        self.addCleanup(p.stdin.close)
        time.sleep(2)
        p.communicate('x' * 1048576)
...
class _SuppressCoreFiles(object):
    ...
    def __enter__(self):
        ...
        if sys.platform == 'darwin':
            value = subprocess.Popen(['/usr/bin/defaults',
             'read',
             'com.apple.CrashReporter',
             'DialogType'], stdout=subprocess.PIPE).communicate()[0]
    ...
    @unittest.skipUnless(hasattr(signal, 'SIGALRM'), 'Requires signal.SIGALRM')
...
class POSIXProcessTestCase(BaseTestCase):
    ...
    def test_exceptions(self):
        with self.assertRaises(OSError) as c:
            p = subprocess.Popen([sys.executable, '-c', ''], cwd='/this/path/does/not/exist')
    ...
    def check_close_std_fds(self, fds):
        ...
        try:
            ...
            out, err = subprocess.Popen([sys.executable, '-c', 'import sys;sys.stdout.write("apple");sys.stdout.flush();sys.stderr.write("orange")'], stdin=stdin, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
    ...
    def check_swap_fds(self, stdin_no, stdout_no, stderr_no):
        ...
        try:
            ...
            try:
                ...
                p = subprocess.Popen([sys.executable, '-c', 'import sys; got = sys.stdin.read();sys.stdout.write("got %s"%got); sys.stderr.write("err")'], stdin=stdin_no, stdout=stdout_no, stderr=stderr_no)
                p.wait()
                for fd in temp_fds:
                    os.lseek(fd, 0, 0)
    ...
    def test_wait_when_sigchild_ignored(self):
        sigchild_ignore = test_support.findfile('sigchild_ignore.py', subdir='subprocessdata')
    ...
    def test_leak_fast_process_del_killed(self):
        ...
        time.sleep(0.2)
        with self.assertRaises(EnvironmentError) as c:
            with subprocess.Popen(['nonexisting_i_hope'], stdout=subprocess.PIPE, stderr=subprocess.PIPE) as proc:
                pass
        self.assertRaises(OSError, os.waitpid, pid, 0)
        self.assertNotIn(ident, [ id(o) for o in subprocess._active ])

    def test_pipe_cloexec(self):
        p1 = subprocess.Popen([sys.executable, '-c', 'import os;os.read(0, 1)'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        p2 = subprocess.Popen([sys.executable, '-c', 'if True:\n                               import os, errno, sys\n                               for fd in %r:\n                                   try:\n                                       os.close(fd)\n                                   except OSError as e:\n                                       if e.errno != errno.EBADF:\n                                           raise\n                                   else:\n                                       sys.exit(1)\n                               sys.exit(0)\n                               ' % [ f.fileno() for f in (p1.stdin, p1.stdout, p1.stderr) ]], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, close_fds=False)
        p1.communicate('foo')
...
class Win32ProcessTestCase(BaseTestCase):
    ...
    def test_invalid_args(self):
        ...
        self.assertRaises(ValueError, subprocess.call, [sys.executable, '-c', 'import sys; sys.exit(47)'], stdout=subprocess.PIPE, close_fds=True)
...
@unittest.skipUnless(getattr(subprocess, '_has_poll', False), 'poll system call not supported')
...
class CommandsWithSpaces(BaseTestCase):
    ...
    def setUp(self):
        ...
        os.write(f, "import sys;sys.stdout.write('%d %s' % (len(sys.argv), [a.lower () for a in sys.argv]))")
        os.close(f)
