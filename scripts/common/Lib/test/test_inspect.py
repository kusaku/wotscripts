# Embedded file name: scripts/common/Lib/test/test_inspect.py
import re
import sys
import types
import unittest
...
class IsTestBase(unittest.TestCase):
    predicates = set([inspect.isbuiltin,
     inspect.isclass,
     inspect.iscode,
     inspect.isframe,
     inspect.isfunction,
    ...
    def istest(self, predicate, exp):
        ...
        for other in self.predicates - set([predicate]):
            if predicate == inspect.isgeneratorfunction and other == inspect.isfunction:
                continue
...
class TestInterpreterStack(IsTestBase):
    ...
    def test_stack(self):
        ...
        self.assertEqual(mod.st[0][1:], (modfile,
         16,
         'eggs',
         ['    st = inspect.stack()\n'],
         0))
        self.assertEqual(mod.st[1][1:], (modfile,
         9,
         'spam',
         ['    eggs(b + d, c + f)\n'],
         0))
        self.assertEqual(mod.st[2][1:], (modfile,
         43,
         'argue',
         ['            spam(a, b, c)\n'],
         0))
        self.assertEqual(mod.st[3][1:], (modfile,
         39,
         'abuse',
         ['        self.argue(a, b, c)\n'],
         0))
    ...
    def test_trace(self):
        ...
        self.assertEqual(git.tr[0][1:], (modfile,
         43,
         'argue',
         ['            spam(a, b, c)\n'],
         0))
        self.assertEqual(git.tr[1][1:], (modfile,
         9,
         'spam',
         ['    eggs(b + d, c + f)\n'],
         0))
        self.assertEqual(git.tr[2][1:], (modfile,
         18,
         'eggs',
         ['    q = y // 0\n'],
         0))
...
class TestRetrievingSourceCode(GetSourceBase):
    ...
    def test_getclasses(self):
        ...
        self.assertEqual(classes, [('FesteringGob', mod.FesteringGob),
         ('MalodorousPervert', mod.MalodorousPervert),
         ('ParrotDroppings', mod.ParrotDroppings),
         ('StupidGit', mod.StupidGit)])
        tree = inspect.getclasstree([ cls[1] for cls in classes ], 1)
        self.assertEqual(tree, [(mod.ParrotDroppings, ()), (mod.StupidGit, ()), [(mod.MalodorousPervert, (mod.StupidGit,)), [(mod.FesteringGob, (mod.MalodorousPervert, mod.ParrotDroppings))]]])

    def test_getfunctions(self):
        functions = inspect.getmembers(mod, inspect.isfunction)
        self.assertEqual(functions, [('eggs', mod.eggs), ('spam', mod.spam)])
    ...
    def test_getsourcefile(self):
        ...
        linecache.cache[co.co_filename] = (1,
         None,
         'None',
         co.co_filename)
...
class TestBuggyCases(GetSourceBase):
    ...
    @unittest.skipIf(not hasattr(unicodedata, '__file__') or unicodedata.__file__[-4:] in ('.pyc', '.pyo'), 'unicodedata is not an external binary module')
...
class TestClassesAndFunctions(unittest.TestCase):
    ...
    def assertArgSpecEquals(self, routine, args_e, varargs_e = None, varkw_e = None, defaults_e = None, formatted = None):
    ...
    def test_getargspec(self):
        ...
        self.assertArgSpecEquals(mod.spam, ['a',
         'b',
         'c',
         'd',
         ['e', ['f']]], 'g', 'h', (3, (4, (5,))), '(a, b, c, d=3, (e, (f,))=(4, (5,)), *g, **h)')
    ...
    def test_getargspec_sublistofone(self):
        with check_py3k_warnings(('tuple parameter unpacking has been removed', SyntaxWarning), ('parenthesized argument names are invalid', SyntaxWarning)):
...
class TestGetcallargsFunctions(unittest.TestCase):
    is_tuplename = re.compile('^\\.\\d+$').match
...
class TestGetcallargsMethods(TestGetcallargsFunctions):
    ...
    def makeCallable(self, signature):
        raise 'self' not in signature or AssertionError
...
class TestGetcallargsUnboundMethods(TestGetcallargsMethods):
    ...
    def assertEqualCallArgs(self, func, call_params_string, locs = None):
        return super(TestGetcallargsUnboundMethods, self).assertEqualCallArgs(*self._getAssertEqualParams(func, call_params_string, locs))

    def assertEqualException(self, func, call_params_string, locs = None):
        return super(TestGetcallargsUnboundMethods, self).assertEqualException(*self._getAssertEqualParams(func, call_params_string, locs))

    def _getAssertEqualParams(self, func, call_params_string, locs = None):
        raise 'inst' not in call_params_string or AssertionError
        locs = dict(locs or {}, inst=self.inst)
        return (func, 'inst,' + call_params_string, locs)


def test_main():
    run_unittest(TestDecorators, TestRetrievingSourceCode, TestOneliners, TestBuggyCases, TestInterpreterStack, TestClassesAndFunctions, TestPredicates, TestGetcallargsFunctions, TestGetcallargsMethods, TestGetcallargsUnboundMethods)
