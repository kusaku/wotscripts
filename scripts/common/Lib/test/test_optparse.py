# Embedded file name: scripts/common/Lib/test/test_optparse.py
import sys
import os
import re
import copy
...
class TestOptionChecks(BaseTest):
    ...
    def test_attr_invalid(self):
        self.assertOptionError('option -b: invalid keyword arguments: bar, foo', ['-b'], {'foo': None,
         'bar': None})
        return
    ...
    def test_no_type_for_action(self):
        self.assertOptionError("option -b: must not supply a type for action 'count'", ['-b'], {'action': 'count',
         'type': 'int'})
    ...
    def test_bad_choices_list(self):
        ...
        self.assertOptionError("option -b/--bad: choices must be a list of strings ('%s' supplied)" % typename, ['-b', '--bad'], {'type': 'choice',
         'choices': 'bad choices'})

    def test_no_choices_for_type(self):
        self.assertOptionError("option -b: must not supply choices for type 'int'", ['-b'], {'type': 'int',
         'choices': 'bad'})

    def test_no_const_for_action(self):
        self.assertOptionError("option -b: 'const' must not be supplied for action 'store'", ['-b'], {'action': 'store',
         'const': 1})

    def test_no_nargs_for_action(self):
        self.assertOptionError("option -b: 'nargs' must not be supplied for action 'count'", ['-b'], {'action': 'count',
         'nargs': 2})

    def test_callback_not_callable(self):
        self.assertOptionError("option -b: callback not callable: 'foo'", ['-b'], {'action': 'callback',
         'callback': 'foo'})
    ...
    def test_callback_args_no_tuple(self):
        self.assertOptionError("option -b: callback_args, if supplied, must be a tuple: not 'foo'", ['-b'], {'action': 'callback',
         'callback': self.dummy,
         'callback_args': 'foo'})

    def test_callback_kwargs_no_dict(self):
        self.assertOptionError("option -b: callback_kwargs, if supplied, must be a dict: not 'foo'", ['-b'], {'action': 'callback',
         'callback': self.dummy,
         'callback_kwargs': 'foo'})

    def test_no_callback_for_action(self):
        self.assertOptionError("option -b: callback supplied ('foo') for non-callback option", ['-b'], {'action': 'store',
         'callback': 'foo'})

    def test_no_callback_args_for_action(self):
        self.assertOptionError('option -b: callback_args supplied for non-callback option', ['-b'], {'action': 'store',
         'callback_args': 'foo'})

    def test_no_callback_kwargs_for_action(self):
        self.assertOptionError('option -b: callback_kwargs supplied for non-callback option', ['-b'], {'action': 'store',
         'callback_kwargs': 'foo'})
...
class TestStandard(BaseTest):
    ...
    def test_empty(self):
        self.assertParseOK([], {'a': None,
         'boo': None,
         'foo': None}, [])
        return

    def test_shortopt_empty_longopt_append(self):
        self.assertParseOK(['-a',
         '',
         '--foo=blah',
         '--foo='], {'a': '',
         'boo': None,
    ...
    def test_short_option_split_one_positional_arg(self):
        (self.assertParseOK(['-a', 'foo', 'bar'], {'a': 'foo',
          'boo': None,
          'foo': None}, ['bar']),)
        return
...
class TestMultipleArgs(BaseTest):
    ...
    def test_nargs_with_positional_args(self):
        self.assertParseOK(['foo',
         ...
         '2.5',
         '-4.3',
         'xyz'], {'point': (1.0, 2.5, -4.3)}, ['foo', 'xyz'])

    def test_nargs_long_opt(self):
        self.assertParseOK(['--point',
         '-1',
         '2.5',
         '-0',
         'xyz'], {'point': (-1.0, 2.5, -0.0)}, ['xyz'])

    def test_nargs_invalid_float_value(self):
        self.assertParseFail(['-p',
         '1.0',
         '2x',
         '3.5'], "option -p: invalid floating-point value: '2x'")
...
class TestMultipleArgsAppend(BaseTest):
    ...
    def test_nargs_append_simple(self):
        self.assertParseOK(['--foo=3', '4'], {'point': None,
         'foo': [(3, 4)]}, [])
        return

    def test_nargs_append_const(self):
        self.assertParseOK(['--zero',
         '--foo',
         '3',
         '4',
         '-z'], {'point': None,
...
class TestExtendAddTypes(BaseTest):
    ...
    def test_filetype_ok(self):
        ...
        self.assertParseOK(['--file', test_support.TESTFN, '-afoo'], {'file': test_support.TESTFN,
         'a': 'foo'}, [])

    def test_filetype_noexist(self):
        self.assertParseFail(['--file', test_support.TESTFN, '-afoo'], '%s: file does not exist' % test_support.TESTFN)

    def test_filetype_notfile(self):
        os.mkdir(test_support.TESTFN)
        self.assertParseFail(['--file', test_support.TESTFN, '-afoo'], '%s: not a regular file' % test_support.TESTFN)
...
class TestCallback(BaseTest):
    ...
    def test_callback(self):
        self.assertParseOK(['-x', '--file=foo'], {'filename': 'foo',
         'x': 42}, [])
...
class TestCallbackMeddleArgs(BaseTest):
    ...
    def test_callback_meddle_args(self):
        self.assertParseOK(['-1',
         'foo',
         '-3',
         'bar',
         'baz',
         'qux'], {'things': [('foo',), ('bar', 'baz', 'qux')]}, [1, 3])
...
class TestCallbackVarArgs(BaseTest):
    ...
    def test_stop_at_option(self):
        self.assertParseOK(['-c', 'foo', '-b'], {'a': None,
         'b': True,
         'c': ['foo']}, [])
        return

    def test_stop_at_invalid_option(self):
        self.assertParseFail(['-c',
         '3',
         '-5',
         '-a'], 'no such option: -5')
...
class TestConflict(ConflictBase):
    ...
    def assertTrueconflict_error(self, func):
        err = self.assertRaises(func, ('-v', '--version'), {'action': 'callback',
         'callback': self.show_version,
         'help': 'show version'}, OptionConflictError, 'option -v/--version: conflicting option string(s): -v')
...
class TestConflictResolve(ConflictBase):
    ...
    def test_conflict_resolve_short_opt(self):
        self.assertParseOK(['-v'], {'verbose': None,
         'show_version': 1}, [])
        return
    ...
    def test_conflict_resolve_long_opts(self):
        self.assertParseOK(['--verbose', '--version'], {'verbose': 1,
         'show_version': 1}, [])
...
class TestHelp(BaseTest):
    ...
    def test_help_unicode(self):
        self.parser = InterceptingOptionParser(usage=SUPPRESS_USAGE)
        self.parser.add_option('-a', action='store_true', help=u'ol\xe9!')
        expect = u'Options:\n  -h, --help  show this help message and exit\n  -a          ol\xe9!\n'
        self.assertHelpEquals(expect)

    def test_help_unicode_description(self):
        self.parser = InterceptingOptionParser(usage=SUPPRESS_USAGE, description=u'ol\xe9!')
        expect = u'ol\xe9!\n\nOptions:\n  -h, --help  show this help message and exit\n'
        self.assertHelpEquals(expect)
...
class TestMatchAbbrev(BaseTest):
    ...
    def test_match_abbrev(self):
        self.assertEqual(_match_abbrev('--f', {'--foz': None,
         '--foo': None,
         '--fie': None,
         '--f': None}), '--f')
        return
...
class TestParseNumber(BaseTest):
    ...
    def test_parse_num_fail(self):
        ...
        self.assertRaises(_parse_num, ('0xOoops', long), {}, ValueError, re.compile("invalid literal for long().*: '?0xOoops'?"))
    ...
    def test_numeric_options(self):
        self.assertParseOK(['-n',
         ...
         '0x20'], {'n': 42,
         'l': 32}, [])
        self.assertParseOK(['-n', '0b0101', '-l010'], {'n': 5,
         'l': 8}, [])
        self.assertParseFail(['-n008'], "option -n: invalid integer value: '008'")
        self.assertParseFail(['-l0b0123'], "option -l: invalid long integer value: '0b0123'")
        self.assertParseFail(['-l', '0x12x'], "option -l: invalid long integer value: '0x12x'")
