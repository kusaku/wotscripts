# Embedded file name: scripts/common/Lib/test/test_linuxaudiodev.py
...
class LinuxAudioDevTests(unittest.TestCase):
    ...
    def test_errors(self):
        ...
        try:
            ...
            self.assertEqual(err.args[0], 'for linear unsigned 16-bit little-endian audio, expected sample size 16, not 8')

        try:
            self.dev.setparameters(rate, 16, nchannels, fmt)
        except ValueError as err:
            self.assertEqual(err.args[0], 'for linear unsigned 8-bit audio, expected sample size 8, not 16')
