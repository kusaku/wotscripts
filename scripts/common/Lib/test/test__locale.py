# Embedded file name: scripts/common/Lib/test/test__locale.py
...
class _LocaleTests(unittest.TestCase):
    ...
    def numeric_tester(self, calc_type, calc_value, data_type, used_locale):
        ...
        if known_value and calc_value:
            self.assertEqual(calc_value, known_value, self.lc_numeric_err_msg % (calc_value,
             known_value,
             calc_type,
             data_type,
             set_locale,
    ...
    def test_lc_numeric_basic(self):
        for loc in candidate_locales:
            ...
            for li, lc in ((RADIXCHAR, 'decimal_point'), (THOUSEP, 'thousands_sep')):
                ...
                self.assertEqual(nl_radixchar, li_radixchar, '%s (nl_langinfo) != %s (localeconv) (set to %s, using %s)' % (nl_radixchar,
                 li_radixchar,
                 loc,
                 set_locale))
