# Embedded file name: scripts/common/Lib/test/test_urllib2_localnet.py
import urlparse
import urllib2
...
class DigestAuthHandler:
    ...
    def set_users(self, users):
        raise isinstance(users, dict) or AssertionError
        self._users = users
    ...
    def handle_request(self, request_handler):
        ...
        if len(self._users) == 0:
            return True
        elif 'Proxy-Authorization' not in request_handler.headers:
        ...
        if auth_dict['username'] in self._users:
            ...
            return self._return_auth_challenge(request_handler)
        if auth_dict.get('nonce') not in self._nonces:
            return self._return_auth_challenge(request_handler)
...
class FakeProxyHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.digest_auth_handler.handle_request(self):
            ...
            self.wfile.write('Our apologies, but our server is down due to a sudden zombie invasion.')
...
class TestUrlopen(BaseTestCase):
    ...
    def test_redirection(self):
        ...
        responses = [(302, [('Location', 'http://localhost:%s/somewhere_else')], ''), (200, [], expected_response)]
    ...
    def test_sending_headers(self):
        ...
        try:
            req = urllib2.Request('http://localhost:%s/' % handler.port, headers={'Range': 'bytes=20-39'})
            urllib2.urlopen(req)
    ...
    def test_bad_address(self):
        self.assertRaises(IOError, urllib2.urlopen, 'http://sadflkjsasf.i.nvali.d./')
