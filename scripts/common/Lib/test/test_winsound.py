# Embedded file name: scripts/common/Lib/test/test_winsound.py
import unittest
...
def has_sound(sound):
    ...
    try:
        if ctypes.windll.winmm.mixerGetNumDevs() is 0:
            return False
        key = _winreg.OpenKeyEx(_winreg.HKEY_CURRENT_USER, 'AppEvents\\Schemes\\Apps\\.Default\\{0}\\.Default'.format(sound))
...
class PlaySoundTest(unittest.TestCase):
    ...
    def test_alias_asterisk(self):
        if _have_soundcard():
            ...
            self.assertRaises(RuntimeError, winsound.PlaySound, 'SystemAsterisk', winsound.SND_ALIAS)
    ...
    def test_alias_exclamation(self):
        if _have_soundcard():
            ...
            self.assertRaises(RuntimeError, winsound.PlaySound, 'SystemExclamation', winsound.SND_ALIAS)
    ...
    def test_alias_exit(self):
        if _have_soundcard():
            ...
            self.assertRaises(RuntimeError, winsound.PlaySound, 'SystemExit', winsound.SND_ALIAS)
    ...
    def test_alias_hand(self):
        if _have_soundcard():
            ...
            self.assertRaises(RuntimeError, winsound.PlaySound, 'SystemHand', winsound.SND_ALIAS)
    ...
    def test_alias_question(self):
        if _have_soundcard():
            ...
            self.assertRaises(RuntimeError, winsound.PlaySound, 'SystemQuestion', winsound.SND_ALIAS)
    ...
    def test_stopasync(self):
        if _have_soundcard():
            winsound.PlaySound('SystemQuestion', winsound.SND_ALIAS | winsound.SND_ASYNC | winsound.SND_LOOP)
            time.sleep(0.5)
            try:
                winsound.PlaySound('SystemQuestion', winsound.SND_ALIAS | winsound.SND_NOSTOP)
            except RuntimeError:
                pass
