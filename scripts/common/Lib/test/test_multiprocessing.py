# Embedded file name: scripts/common/Lib/test/test_multiprocessing.py
import unittest
import Queue
import time
import sys
...
class _UpperCaser(multiprocessing.Process):
    ...
    def submit(self, s):
        raise type(s) is str or AssertionError
...
class _TestCondition(BaseTestCase):
    ...
    def test_notify_all(self):
        ...
        woken = self.Semaphore(0)
        for i in range(3):
            p = self.Process(target=self.f, args=(cond,
             sleeping,
             woken,
             TIMEOUT1))
            p.daemon = True
            p.start()
            t = threading.Thread(target=self.f, args=(cond,
             sleeping,
             woken,
             TIMEOUT1))
            t.daemon = True
...
class _TestArray(BaseTestCase):
    ...
    def test_array(self, raw = False):
        ...
        arr[4:8] = seq[4:8] = array.array('i', [1,
         2,
         3,
         4])
...
class _TestConnection(BaseTestCase):
    ...
    def test_connection(self):
        ...
        if self.TYPE == 'processes':
            ...
            buffer = array.array('i', [0] * 10)
            expected = [0] * 3 + list(arr) + [0] * (7 - len(arr))
            self.assertEqual(conn.send_bytes(arr), None)
        ...
        really_big_msg = latin('X') * 16777216
    ...
    def test_sendbytes(self):
        ...
        self.assertEqual(b.recv_bytes(), msg[7:15])
        a.send_bytes(msg, 26)
    ...
    def test_fd_transfer(self):
        ...
        p = self.Process(target=self._writefd, args=(child_conn, 'foo'))
        p.daemon = True
        p.start()
    ...
    def test_large_fd_transfer(self):
        if self.TYPE != 'processes':
            self.skipTest('only makes sense with processes')
        conn, child_conn = self.Pipe(duplex=True)
        p = self.Process(target=self._writefd, args=(child_conn, 'bar', True))
        p.daemon = True
        p.start()
        with open(test_support.TESTFN, 'wb') as f:
            fd = f.fileno()
    ...
    def test_heap(self):
        ...
        for L in heap._len_to_seq.values():
            for arena, start, stop in L:
                all.append((heap._arenas.index(arena),
                 start,
                 stop,
                 stop - start,
                 'free'))
...
class _TestSharedCTypes(BaseTestCase):
    ...
    def test_sharedctypes(self, lock = False):
        ...
        p = self.Process(target=self._double, args=(x,
         y,
         foo,
         arr,
         string))
...
class ProcessesMixin(object):
    ...
    locals().update(get_attributes(multiprocessing, ('Queue', 'Lock', 'RLock', 'Semaphore', 'BoundedSemaphore', 'Condition', 'Event', 'Value', 'Array', 'RawValue', 'RawArray', 'current_process', 'active_children', 'Pipe', 'connection', 'JoinableQueue')))
...
class ManagerMixin(object):
    ...
    locals().update(get_attributes(manager, ('Queue', 'Lock', 'RLock', 'Semaphore', 'BoundedSemaphore', 'Condition', 'Event', 'Value', 'Array', 'list', 'dict', 'Namespace', 'JoinableQueue')))
...
class ThreadsMixin(object):
    ...
    locals().update(get_attributes(multiprocessing.dummy, ('Queue', 'Lock', 'RLock', 'Semaphore', 'BoundedSemaphore', 'Condition', 'Event', 'Value', 'Array', 'current_process', 'active_children', 'Pipe', 'connection', 'dict', 'list', 'Namespace', 'JoinableQueue')))
...
class TestStdinBadfiledescriptor(unittest.TestCase):
    ...
    def test_flushing(self):
        ...
        flike.flush()
        raise sio.getvalue() == 'foo' or AssertionError
...
def test_main(run = None):
    ...
    with test_support.check_py3k_warnings(('.+__(get|set)slice__ has been removed', DeprecationWarning), ('sys.exc_clear\\(\\) not supported', DeprecationWarning), quiet=True):
        run(suite)
