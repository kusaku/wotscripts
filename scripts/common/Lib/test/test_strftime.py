# Embedded file name: scripts/common/Lib/test/test_strftime.py
"""
...
class StrftimeTest(unittest.TestCase):
    ...
    def _update_variables(self, now):
        ...
        if now[3] < 12:
            ...
            self.ampm = '(PM|pm)'
        self.jan1 = time.localtime(time.mktime((now[0],
         1,
         1,
         0,
         0,
    ...
    def strftest1(self, now):
        ...
        now = self.now
        expectations = (('%a', calendar.day_abbr[now[6]], 'abbreviated weekday name'),
         ...
         ('%U', '%02d' % ((now[7] + self.jan1[6]) // 7), 'week number of the year (Sun 1st)'),
         ('%w', '0?%d' % ((1 + now[6]) % 7), 'weekday as a number (Sun 1st)'),
         ('%W', '%02d' % ((now[7] + (self.jan1[6] - 1) % 7) // 7), 'week number of the year (Mon 1st)'),
        ...
        for e in expectations:
            ...
            if not result or result[0] == '%':
                ...
                self.fail('Conflict for %s (%s): expected %s, but got %s' % (e[0],
                 e[2],
                 e[1],
                 result))
    ...
    def strftest2(self, now):
        ...
        now = self.now
        nonstandard_expectations = (('%c', fixasctime(time.asctime(now)), 'near-asctime() format'),
         ('%x', '%02d/%02d/%02d' % (now[1], now[2], now[0] % 100), '%m/%d/%y %H:%M:%S'),
