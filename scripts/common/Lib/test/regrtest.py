# Embedded file name: scripts/common/Lib/test/regrtest.py
"""
Usage:
...
def main(tests = None, testdir = None, verbose = 0, quiet = False, exclude = False, single = False, randomize = False, fromfile = None, findleaks = False, use_resources = None, trace = False, coverdir = 'coverage', runleaks = False, huntrleaks = False, verbose2 = False, print_slow = False, random_seed = None, use_mp = None, verbose3 = False, forever = False, header = False):
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'hvqxsSrf:lu:t:TD:NLR:FwWM:j:', ['help',
         'verbose',
         'verbose2',
         'verbose3',
         'quiet',
    ...
    for o, a in opts:
        if o in ('-h', '--help'):
            ...
            print >> sys.stderr, 'No handler for option {}.  Please report this as a bug at http://bugs.python.org.'.format(o)
            sys.exit(1)
    ...
    if verbose or header or not (quiet or single or tests or args):
        ...
        print ' '.join(sys.version.split())
        print '==  ', platform.platform(aliased=True),
        print '%s-endian' % sys.byteorder
        print '==  ', os.getcwd()
    ...
    if use_mp:
        ...
        debug_output_pat = re.compile('\\[\\d+ refs\\]$')
        output = Queue()

        def tests_and_args():
            for test in tests:
                args_tuple = ((test, verbose, quiet), dict(huntrleaks=huntrleaks, use_resources=use_resources))
        ...
        def work():
            try:
                while True:
                    ...
                    popen = Popen(base_cmd + ['--slaveargs', json.dumps(args_tuple)], stdout=PIPE, stderr=PIPE, universal_newlines=True, close_fds=os.name != 'nt')
        ...
        try:
            while finished < use_mp:
                ...
                if not (result[0] == INTERRUPTED and result[1] == 'KeyboardInterrupt'):
        ...
        for test in tests:
            if not quiet:
                ...
                sys.stdout.flush()
            if trace:
                tracer.runctx('runtest(test, verbose, quiet)', globals=globals(), locals=vars())
            else:
                try:
    ...
    if bad:
        ...
        if environment_changed:
            print '{} altered the execution environment:'.format(count(len(environment_changed), 'test'))
...
class saved_test_environment():
    ...
    resources = ('sys.argv', 'cwd', 'sys.stdin', 'sys.stdout', 'sys.stderr', 'os.environ', 'sys.path', 'asyncore.socket_map')
    ...
    def __enter__(self):
        self.saved_values = dict(((name, get()) for name, get, restore in self.resource_info()))
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        for name, get, restore in self.resource_info():
            ...
            if current != original:
                ...
                restore(original)
                if not self.quiet:
                    print >> sys.stderr, 'Warning -- {} was modified by {}'.format(name, self.testname)
                    if self.verbose > 1:
                        print >> sys.stderr, '  Before: {}\n  After:  {} '.format(original, current)
...
def cleanup_test_droppings(testname, verbose):
    ...
    for name in (test_support.TESTFN, 'db_home'):
        ...
        try:
            ...
            print >> sys.stderr, "%r left behind %s %r and it couldn't be removed: %s" % (testname,
             kind,
             name,
             msg)
...
def printlist(x, width = 70, indent = 4):
    ...
    blanks = ' ' * indent
    print fill(' '.join((str(elt) for elt in sorted(x))), width, initial_indent=blanks, subsequent_indent=blanks)
...
class _ExpectedSkips():
    ...
    def __init__(self):
        ...
        if sys.platform in _expectations:
            ...
            elif len(u'\x00'.encode('unicode-internal')) == 4:
                self.expected.add('test_macostools')
            if sys.platform != 'win32':
                WIN_ONLY = ['test_unicode_file',
                 'test_winreg',
                 'test_winsound',
                 'test_startfile',
                 'test_sqlite',
    ...
    def getexpected(self):
        ...
        """
        raise self.isvalid() or AssertionError
        return self.expected


__file__ = __name__ == '__main__' and os.path.abspath(__file__)
if not __file__ == os.path.abspath(sys.argv[0]):
    raise AssertionError
