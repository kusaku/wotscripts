# Embedded file name: scripts/common/Lib/test/test_zipfile.py
try:
    import zlib
except ImportError:
    zlib = None
...
class TestsWithSourceFile(unittest.TestCase):
    ...
    def setUp(self):
        self.line_gen = [ 'Zipfile test line %d. random float: %f' % (i, random()) for i in xrange(FIXEDTEST_SIZE) ]
        self.data = '\n'.join(self.line_gen) + '\n'
    ...
    def test_ignores_stuff_appended_past_comments(self):
        with zipfile.ZipFile(TESTFN2, 'w', zipfile.ZIP_STORED) as zipfp:
            zipfp.comment = 'this is a comment'
            zipfp.write(TESTFN, TESTFN)
        with open(TESTFN2, 'a') as f:
            f.write('abcdef\r\n')
        with zipfile.ZipFile(TESTFN2, 'r') as zipfp:
            self.assertIsInstance(zipfp, zipfile.ZipFile)
            self.assertEqual(zipfp.comment, 'this is a comment')
    ...
    def zip_test_writestr_permissions(self, f, compression):
        ...
        with zipfile.ZipFile(f, 'r') as zipfp:
            ...
            self.assertEqual(zinfo.external_attr, 25165824)
...
class OtherTests(unittest.TestCase):
    zips_with_bad_crc = {zipfile.ZIP_STORED: 'PK\x03\x04\x14\x00\x00\x00\x00\x00 \x8b\x8a;:r\xab\xff\x0c\x00\x00\x00\x0c\x00\x00\x00\x05\x00\x00\x00afilehello,AworldPK\x01\x02\x14\x03\x14\x00\x00\x00\x00\x00 \x8b\x8a;:r\xab\xff\x0c\x00\x00\x00\x0c\x00\x00\x00\x05\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x80\x01\x00\x00\x00\x00afilePK\x05\x06\x00\x00\x00\x00\x01\x00\x01\x003\x00\x00\x00/\x00\x00\x00\x00\x00',
     zipfile.ZIP_DEFLATED: 'PK\x03\x04\x14\x00\x00\x00\x08\x00n}\x0c=FAKE\x10\x00\x00\x00n\x00\x00\x00\x05\x00\x00\x00afile\xcbH\xcd\xc9\xc9W(\xcf/\xcaI\xc9\xa0=\x13\x00PK\x01\x02\x14\x03\x14\x00\x00\x00\x08\x00n}\x0c=FAKE\x10\x00\x00\x00n\x00\x00\x00\x05\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x80\x01\x00\x00\x00\x00afilePK\x05\x06\x00\x00\x00\x00\x01\x00\x01\x003\x00\x00\x003\x00\x00\x00\x00\x00'}
    ...
    def test_comments(self):
        ...
        comment2 = ''.join([ '%d' % (i ** 3 % 10) for i in xrange(65535) ])
        with zipfile.ZipFile(TESTFN, mode='w') as zipf:
    ...
    def check_read_return_size(self, compression):
        for test_size in (1, 4095, 4096, 4097, 16384):
            ...
            junk = ''.join((struct.pack('B', randint(0, 255)) for x in range(file_size)))
