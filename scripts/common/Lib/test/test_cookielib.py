# Embedded file name: scripts/common/Lib/test/test_cookielib.py
...
class DateTimeTests(TestCase):
    ...
    def test_time2isoz(self):
        ...
        bz = time2isoz(500000)
        for text in (az, bz):
            self.assertTrue(re.search('^\\d{4}-\\d\\d-\\d\\d \\d\\d:\\d\\d:\\d\\dZ$', text), 'bad time2isoz format: %s %s' % (az, bz))
    ...
    def test_http2time_formats(self):
        ...
        for s in tests:
            ...
            self.assertTrue(t == t2 == t3 == test_t, "'%s'  =>  %s, %s, %s (%s)" % (s,
             t,
             t2,
             t3,
             test_t))
    ...
    def test_http2time_garbage(self):
        ...
        for test in ['',
         ...
         '01-01-1980 00:61:00',
         '01-01-1980 00:00:62']:
            self.assertTrue(http2time(test) is None, 'http2time(%s) is not None\nhttp2time(test) %s' % (test, http2time(test)))
...
class HeaderTests(TestCase):
    ...
    def test_split_header_words(self):
        ...
        tests = [('foo', [[('foo', None)]]),
         ...
         ('foo; bar=baz, spam=, foo="\\,\\;\\"", bar= ', [[('foo', None), ('bar', 'baz')],
           [('spam', '')],
           [('foo', ',;"')],
           [('bar', '')]])]
...
class CookieTests(TestCase):
    ...
    def test_ns_parser(self):
        ...
        self.assertTrue(cookie.has_nonstandard_attr('blArgh') and not cookie.has_nonstandard_attr('blargh'))
    ...
    def test_request_path(self):
        ...
        req = Request('http://www.example.com/rheum/rhaponticum;foo=bar;sing=song?apples=pears&spam=eggs#ni')
        self.assertEqual(request_path(req), '/rheum/rhaponticum;foo=bar;sing=song')
        req = Request('http://www.example.com/rheum/rhaponticum?apples=pears&spam=eggs#ni')
    ...
    def test_domain_block(self):
        ...
        req = Request('http://www.roadrunner.net/')
        c.add_cookie_header(req)
        self.assertTrue(req.has_header('Cookie') and req.has_header('Cookie2'))
        c.clear()
    ...
    def test_secure(self):
        ...
        for ns in (True, False):
            for whitespace in (' ', ''):
                ...
                self.assertTrue(not c._cookies['www.acme.com']['/']['foo1'].secure, 'non-secure cookie registered secure')
    ...
    def test_port_mirror(self):
        ...
        h = interact_2965(c, url)
        self.assertTrue(re.search('\\$Port([^=]|$)', h), 'port with no value not returned with no value')
        c = CookieJar(pol)
        url = 'http://foo.bar.com/'
    ...
    def test_no_return_comment(self):
        ...
        h = interact_2965(c, url)
        self.assertTrue('Comment' not in h, 'Comment or CommentURL cookie-attributes returned to server')
    ...
    def test_parse_ns_headers(self):
        from cookielib import parse_ns_headers
        self.assertEqual(parse_ns_headers(['foo=bar; path=/; domain']), [[('foo', 'bar'),
          ('path', '/'),
          ('domain', None),
          ('version', '0')]])
...
class LWPCookieTests(TestCase):
    ...
    def test_netscape_example_2(self):
        ...
        c.add_cookie_header(req)
        self.assertTrue(re.search('PART_NUMBER=RIDING_ROCKET_0023;\\s*PART_NUMBER=ROCKET_LAUNCHER_0001', req.get_header('Cookie')))
    ...
    def test_ietf_example_1(self):
        ...
        self.assertTrue(re.search('^\\$Version="?1"?; Customer="?WILE_E_COYOTE"?; \\$Path="/acme"$', cookie))
        cookie = interact_2965(c, 'http://www.acme.com/acme/shipping', 'Shipping="FedEx"; Version="1"; Path="/acme"')
        self.assertTrue(re.search('^\\$Version="?1"?;', cookie))
        self.assertTrue(re.search('Part_Number="?Rocket_Launcher_0001"?;\\s*\\$Path="\\/acme"', cookie))
        self.assertTrue(re.search('Customer="?WILE_E_COYOTE"?;\\s*\\$Path="\\/acme"', cookie))
        cookie = interact_2965(c, 'http://www.acme.com/acme/process')
        self.assertTrue(re.search('Shipping="?FedEx"?;\\s*\\$Path="\\/acme"', cookie) and 'WILE_E_COYOTE' in cookie)
    ...
    def test_ietf_example_2(self):
        ...
        self.assertTrue(re.search('Riding_Rocket_0023.*Rocket_Launcher_0001', cookie))
    ...
    def test_netscape_misc(self):
        ...
        req = Request('http://foo.bar.acme.com/foo')
        c.add_cookie_header(req)
        self.assertTrue('PART_NUMBER=3,4' in req.get_header('Cookie') and 'Customer=WILE_E_COYOTE' in req.get_header('Cookie'))
    ...
    def test_session_cookies(self):
        ...
        self.assertTrue(not (counter['perm_after'] != counter['perm_before'] or counter['session_after'] != 0 or counter['session_before'] == 0))
