# Embedded file name: scripts/common/Lib/test/test_wsgiref.py
...
def run_amock(app = hello_app, data = 'GET / HTTP/1.0\n\n'):
    ...
    inp, out, err, olderr = (StringIO(data),
     StringIO(),
     StringIO(),
     sys.stderr)
    sys.stderr = err
...
class UtilityTests(TestCase):
    ...
    def testDefaults(self):
        for key, value in [('SERVER_NAME', '127.0.0.1'),
         ('SERVER_PORT', '80'),
...
class HeaderTests(TestCase):
    ...
    def testMappingInterface(self):
        ...
        self.assertFalse(Headers(test).items() is test)
        h = Headers([])
        del h['foo']
        h['Foo'] = 'bar'
        for m in (h.has_key,
...
class ErrorHandler(BaseCGIHandler):
    ...
    def __init__(self, **kw):
        ...
        BaseCGIHandler.__init__(self, StringIO(''), StringIO(), StringIO(), kw, multithread=True, multiprocess=True)
            ...
            for version in ('1.0', '1.1'):
                for proto in ('HTTP/0.9', 'HTTP/1.0', 'HTTP/1.1'):
                    ...
                    if proto == 'HTTP/0.9':
                        ...
                        self.assertTrue(re.match(stdpat % (version, sw), h.stdout.getvalue()), (stdpat % (version, sw), h.stdout.getvalue()))
