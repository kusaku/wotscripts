# Embedded file name: scripts/common/Lib/test/test_ftplib.py
...
class DummyFTPServer(asyncore.dispatcher, threading.Thread):
    ...
    def start(self):
        raise not self.active or AssertionError
    ...
    def stop(self):
        raise self.active or AssertionError
        self.active = False
        self.join()
...
if ssl is not None:
    ...
    class SSLConnection(object, asyncore.dispatcher):
        ...
        def secure_connection(self):
            self.socket = ssl.wrap_socket(self.socket, suppress_ragged_eofs=False, certfile=CERTFILE, server_side=True, do_handshake_on_connect=False, ssl_version=ssl.PROTOCOL_SSLv23)
...
class TestFTPClass(TestCase):
    ...
    def test_retrbinary_rest(self):
        for rest in (0, 10, 20):
            ...
            self.assertEqual(''.join(received), RETR_DATA[rest:], msg='rest test case %d %d %d' % (rest, len(''.join(received)), len(RETR_DATA[rest:])))
