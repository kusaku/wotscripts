# Embedded file name: scripts/common/Lib/test/test_posixpath.py
import unittest
...
class PosixPathTest(unittest.TestCase):
    ...
    def test_samestat(self):
        ...
        try:
            f.write('foo')
            f.close()
            self.assertIs(posixpath.samestat(os.stat(test_support.TESTFN + '1'), os.stat(test_support.TESTFN + '1')), True)
            if hasattr(os, 'symlink'):
                if hasattr(os, 'symlink'):
                    os.symlink(test_support.TESTFN + '1', test_support.TESTFN + '2')
                    self.assertIs(posixpath.samestat(os.stat(test_support.TESTFN + '1'), os.stat(test_support.TESTFN + '2')), True)
                    os.remove(test_support.TESTFN + '2')
                f = open(test_support.TESTFN + '2', 'wb')
                f.write('bar')
                f.close()
                self.assertIs(posixpath.samestat(os.stat(test_support.TESTFN + '1'), os.stat(test_support.TESTFN + '2')), False)
        finally:
            if not f.close():
                f.close()
