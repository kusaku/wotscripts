# Embedded file name: scripts/common/Lib/test/test_strptime.py
...
class LocaleTime_Tests(unittest.TestCase):
    ...
    def compare_against_time(self, testing, directive, tuple_position, error_msg):
        ...
        self.assertTrue(comparison == strftime_output, '%s: position within tuple incorrect; %s != %s' % (error_msg, comparison, strftime_output))
    ...
    def test_am_pm(self):
        ...
        if self.time_tuple[3] < 12:
            ...
            position = 1
        self.assertTrue(strftime_output == self.LT_ins.am_pm[position], 'AM/PM representation in the wrong position within the tuple')
    ...
    def test_date_time(self):
        ...
        self.assertTrue(strftime_output == time.strftime(self.LT_ins.LC_date_time, magic_date), 'LC_date_time incorrect')
        strftime_output = time.strftime('%x', magic_date)
        self.assertTrue(strftime_output == time.strftime(self.LT_ins.LC_date, magic_date), 'LC_date incorrect')
        strftime_output = time.strftime('%X', magic_date)
        self.assertTrue(strftime_output == time.strftime(self.LT_ins.LC_time, magic_date), 'LC_time incorrect')
...
class TimeRETests(unittest.TestCase):
    ...
    def test_compile(self):
        ...
        self.assertTrue(found.group('a') == self.locale_time.a_weekday[4] and found.group('b') == self.locale_time.a_month[4], "re object couldn't find the abbreviated weekday month in '%s' using '%s'; group 'a' = '%s', group 'b' = %s'" % (found.string,
         found.re.pattern,
         found.group('a'),
         found.group('b')))
    ...
    def test_blankpattern(self):
        ...
        self.assertTrue(_strptime.TimeRE(test_locale).pattern('%Z') == '', "with timezone == ('',''), TimeRE().pattern('%Z') != ''")
...
class StrptimeTests(unittest.TestCase):
    ...
    def helper(self, directive, position):
        ...
        self.assertTrue(strp_output[position] == self.time_tuple[position], "testing of '%s' directive failed; '%s' -> %s != %s" % (directive,
         strf_output,
    ...
    def test_bad_timezone(self):
        ...
        try:
            ...
            self.assertEqual(tz_value, -1, '%s lead to a timezone value of %s instead of -1 when time.daylight set to %s and passing in %s' % (time.tzname,
             tz_value,
             time.daylight,
             tz_name))
        finally:
...
class CalculationTests(unittest.TestCase):
    ...
    def test_gregorian_calculation(self):
        ...
        result = _strptime._strptime_time(time.strftime(format_string, self.time_tuple), format_string)
        self.assertTrue(result.tm_year == self.time_tuple.tm_year and result.tm_mon == self.time_tuple.tm_mon and result.tm_mday == self.time_tuple.tm_mday, 'Calculation of Gregorian date failed;%s-%s-%s != %s-%s-%s' % (result.tm_year,
         result.tm_mon,
         result.tm_mday,
    ...
    def test_week_of_year_and_day_of_week_calculation(self):
        ...
        def test_helper(ymd_tuple, test_reason):
            for directive in ('W', 'U'):
                ...
                self.assertTrue(strp_output[:3] == ymd_tuple, "%s(%s) test failed w/ '%s': %s != %s (%s != %s)" % (test_reason,
                 directive,
                 strp_input,
                 strp_output[:3],
                 ymd_tuple,
...
def test_main():
    test_support.run_unittest(getlang_Tests, LocaleTime_Tests, TimeRETests, StrptimeTests, Strptime12AMPMTests, JulianTests, CalculationTests, CacheTests)
