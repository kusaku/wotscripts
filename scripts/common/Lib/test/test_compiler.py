# Embedded file name: scripts/common/Lib/test/test_compiler.py
...
class CompilerTest(unittest.TestCase):
    ...
    def testKeywordAfterStarargs(self):
        ...
        def f(*args, **kwargs):
            self.assertEqual((args, kwargs), ((2, 3), {'x': 1,
              'y': 4}))
    ...
    def _check_lineno(self, node):
        if node.__class__ not in NOLINENO:
            self.assertIsInstance(node.lineno, int, 'lineno=%s on %s' % (node.lineno, node.__class__))
            self.assertTrue(node.lineno > 0, 'lineno=%s on %s' % (node.lineno, node.__class__))
    ...
    def testGenExp(self):
        c = compiler.compile('list((i,j) for i in range(3) if i < 3           for j in range(4) if j > 2)', '<string>', 'eval')
    ...
    def testSetComp(self):
        ...
        self.assertEqual(eval(c), {1, 2, 3})
        c = compiler.compile('{x * y for x in range(3) if x != 0       for y in range(4) if y != 0}', '<string>', 'eval')
    ...
    def testDictComp(self):
        ...
        self.assertEqual(eval(c), {1: 2,
         2: 3,
         3: 4})
        c = compiler.compile('{(x, y) : y for x in range(2) if x != 0            for y in range(3) if y != 0}', '<string>', 'eval')
        self.assertEqual(eval(c), {(1, 2): 2,
         (1, 1): 1})
    ...
    def testSourceCodeEncodingsError(self):
        sjis = "print '\x83p\x83C\x83\\\x83\x93', '\n"
