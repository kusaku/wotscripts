# Embedded file name: scripts/common/Lib/test/test_importlib.py
import contextlib
import imp
import importlib
import sys
...
def uncache(*names):
    ...
    for name in names:
        if name in ('sys', 'marshal', 'imp'):
            raise ValueError('cannot uncache {0} as it will break _importlib'.format(name))
        try:
