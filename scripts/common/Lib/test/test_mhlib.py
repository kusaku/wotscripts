# Embedded file name: scripts/common/Lib/test/test_mhlib.py
"""
...
def writeContext(folder):
    folder = normF(folder)
    writeFile(os.path.join(_mhpath, 'context'), 'Current-Folder: %s\n' % folder)


def writeCurMessage(folder, cur):
    folder = normF(folder)
    writeFile(os.path.join(_mhpath, folder, '.mh_sequences'), 'cur: %s\n' % cur)
...
class MhlibTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        writeMessage('inbox', 1, {'From': 'Mrs. Premise',
         'To': 'Mrs. Conclusion',
         'Date': '18 July 2001'}, 'Hullo, Mrs. Conclusion!\n')
        writeMessage('inbox', 2, {'From': 'Mrs. Conclusion',
         'To': 'Mrs. Premise',
         'Date': '29 July 2001'}, 'Hullo, Mrs. Premise!\n')
        for i in range(5, 101) + range(101, 201, 2):
            writeMessage('wide', i, {'From': 'nowhere',
             'Subject': 'message #%s' % i}, 'This is message number %s\n' % i)
