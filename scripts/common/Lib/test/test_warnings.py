# Embedded file name: scripts/common/Lib/test/test_warnings.py
...
class FilterTests(object):
    ...
    def test_inheritance(self):
        with original_warnings.catch_warnings(module=self.module) as w:
            ...
            self.assertRaises(UserWarning, self.module.warn, 'FilterTests.test_inheritance', UserWarning)

    def test_ordering(self):
        with original_warnings.catch_warnings(record=True, module=self.module) as w:
            self.module.resetwarnings()
            self.module.filterwarnings('ignore', category=UserWarning)
            self.module.filterwarnings('error', category=UserWarning, append=True)
            del w[:]
            try:
...
class WarnTests(unittest.TestCase):
    ...
    def test_warn_explicit_type_errors(self):
        self.assertRaises(TypeError, self.module.warn_explicit, None, UserWarning, None, None)
        self.assertRaises(TypeError, self.module.warn_explicit, None, None, None, 1)
        self.assertRaises((TypeError, AttributeError), self.module.warn_explicit, None, Warning, None, 1, registry=42)
        return
...
class WCmdLineTests(unittest.TestCase):
    ...
    def test_improper_input(self):
        with original_warnings.catch_warnings(module=self.module):
            self.assertRaises(self.module._OptionError, self.module._setoption, '1:2:3:4:5:6')
            self.assertRaises(self.module._OptionError, self.module._setoption, 'bogus::Warning')
            self.assertRaises(self.module._OptionError, self.module._setoption, 'ignore:2::4:-5')
            self.module._setoption('error::Warning::0')
            self.assertRaises(UserWarning, self.module.warn, 'convert to error')

    def test_improper_option(self):
        rc, out, err = assert_python_ok('-Wxxx', '-c', 'pass')
        self.assertIn("Invalid -W option ignored: invalid action: 'xxx'", err)

    def test_warnings_bootstrap(self):
        rc, out, err = assert_python_ok('-Wi', '-c', "import sys; sys.modules['warnings'].warn('foo', RuntimeWarning)")
...
class _WarningsTests(BaseTest):
    ...
    def test_default_action(self):
        ...
        try:
            with original_warnings.catch_warnings(record=True, module=self.module) as w:
                self.module.resetwarnings()
                registry = {}
                self.module.warn_explicit(message, UserWarning, '<test>', 42, registry=registry)
                self.assertEqual(w[-1].message, message)
                ...
                __warningregistry__ = {}
                registry = {}
                self.module.warn_explicit(message, UserWarning, '<test>', 43, registry=registry)
                self.assertEqual(w[-1].message, message)
                self.assertEqual(len(w), 1)
                self.assertEqual(len(registry), 1)
                del w[:]
                ...
                __warningregistry__ = {}
                registry = {}
                self.module.warn_explicit(message, UserWarning, '<test>', 44, registry=registry)
                self.assertEqual(len(w), 0)
        finally:
    ...
    def test_show_warning_output(self):
        ...
        raise expected_line or AssertionError
...
class WarningsDisplayTests(unittest.TestCase):
    ...
    def test_formatwarning(self):
        ...
        expect = format % (file_name,
         ...
         message,
         file_line)
        self.assertEqual(expect, self.module.formatwarning(message, category, file_name, line_num))
        ...
        expect = format % (file_name,
         ...
         message,
         file_line)
        self.assertEqual(expect, self.module.formatwarning(message, category, file_name, line_num, file_line))
    ...
    def test_showwarning(self):
        ...
        expect = self.module.formatwarning(message, category, file_name, line_num)
        self.module.showwarning(message, category, file_name, line_num, file_object)
        self.assertEqual(file_object.getvalue(), expect)
        expected_file_line += 'for the win!'
        expect = self.module.formatwarning(message, category, file_name, line_num, expected_file_line)
        file_object = StringIO.StringIO()
        self.module.showwarning(message, category, file_name, line_num, file_object, expected_file_line)
...
class EnvironmentVariableTests(BaseTest):
    ...
    def test_single_warning(self):
        ...
        p = subprocess.Popen([sys.executable, '-c', 'import sys; sys.stdout.write(str(sys.warnoptions))'], stdout=subprocess.PIPE, env=newenv)
    ...
    def test_comma_separated_warnings(self):
        ...
        p = subprocess.Popen([sys.executable, '-c', 'import sys; sys.stdout.write(str(sys.warnoptions))'], stdout=subprocess.PIPE, env=newenv)
        self.assertEqual(p.communicate()[0], "['ignore::DeprecationWarning', 'ignore::UnicodeWarning']")
        self.assertEqual(p.wait(), 0)
    ...
    def test_envvar_and_command_line(self):
        ...
        p = subprocess.Popen([sys.executable,
         ...
         '-c',
         'import sys; sys.stdout.write(str(sys.warnoptions))'], stdout=subprocess.PIPE, env=newenv)
        self.assertEqual(p.communicate()[0], "['ignore::UnicodeWarning', 'ignore::DeprecationWarning']")
        self.assertEqual(p.wait(), 0)
...
def test_main():
    ...
    test_support.run_unittest(CFilterTests, PyFilterTests, CWarnTests, PyWarnTests, CWCmdLineTests, PyWCmdLineTests, _WarningsTests, CWarningsDisplayTests, PyWarningsDisplayTests, CCatchWarningTests, PyCatchWarningTests, CEnvironmentVariableTests, PyEnvironmentVariableTests)
