# Embedded file name: scripts/common/Lib/test/test_audioop.py
import audioop
import unittest
from test.test_support import run_unittest
endian = 'big' if audioop.getsample('\x00\x01', 2, 0) == 1 else 'little'
...
def gendata2():
    if endian == 'big':
        return '\x00\x00\x00\x01\x00\x02'
    else:
        return '\x00\x00\x01\x00\x02\x00'


def gendata4():
    if endian == 'big':
        return '\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x02'
    else:
        return '\x00\x00\x00\x00\x01\x00\x00\x00\x02\x00\x00\x00'


data = [gendata1(), gendata2(), gendata4()]
INVALID_DATA = [('abc', 0), ('abc', 2), ('abc', 4)]
...
class TestAudioop(unittest.TestCase):
    ...
    def test_adpcm2lin(self):
        self.assertEqual(audioop.adpcm2lin('\x00\x00', 1, None), ('\x00\x00\x00\x00', (0, 0)))
        self.assertEqual(audioop.adpcm2lin('\x00\x00', 2, None), ('\x00\x00\x00\x00\x00\x00\x00\x00', (0, 0)))
        self.assertEqual(audioop.adpcm2lin('\x00\x00', 4, None), ('\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00', (0, 0)))
        return

    def test_lin2adpcm(self):
        self.assertEqual(audioop.lin2adpcm('\x00\x00\x00\x00', 1, None), ('\x00\x00', (0, 0)))
        return
    ...
    def test_alaw2lin(self):
        ...
        if endian == 'big':
            self.assertEqual(audioop.alaw2lin(d, 2), '\x00\x08\x01\x08\x02\x10')
            self.assertEqual(audioop.alaw2lin(d, 4), '\x00\x08\x00\x00\x01\x08\x00\x00\x02\x10\x00\x00')
        else:
            self.assertEqual(audioop.alaw2lin(d, 2), '\x08\x00\x08\x01\x10\x02')
            self.assertEqual(audioop.alaw2lin(d, 4), '\x00\x00\x08\x00\x00\x00\x08\x01\x00\x00\x10\x02')
    ...
    def test_ulaw2lin(self):
        ...
        if endian == 'big':
            self.assertEqual(audioop.ulaw2lin(d, 2), '\x00\x00\x01\x04\x02\x0c')
            self.assertEqual(audioop.ulaw2lin(d, 4), '\x00\x00\x00\x00\x01\x04\x00\x00\x02\x0c\x00\x00')
        else:
            self.assertEqual(audioop.ulaw2lin(d, 2), '\x00\x00\x04\x01\x0c\x02')
            self.assertEqual(audioop.ulaw2lin(d, 4), '\x00\x00\x00\x00\x00\x00\x04\x01\x00\x00\x0c\x02')
    ...
    def test_ratecv(self):
        ...
        self.assertEqual(d1 + d2, '\x00\x00\x01\x01\x02\x01\x00\x00\x01\x01\x02')
        return

    def test_reverse(self):
        self.assertEqual(audioop.reverse(data[0], 1), '\x02\x01\x00')
    ...
    def test_negativelen(self):
        self.assertRaises(audioop.error, audioop.findmax, ''.join((chr(x) for x in xrange(256))), -2392392)
