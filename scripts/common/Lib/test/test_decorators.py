# Embedded file name: scripts/common/Lib/test/test_decorators.py
import unittest
...
class DbcheckError(Exception):
    ...
    def __init__(self, exprstr, func, args, kwds):
        Exception.__init__(self, 'dbcheck %r failed (func=%s args=%s kwds=%s)' % (exprstr,
         func,
         args,
         kwds))
        ...
        class NameLookupTracer(object):
            ...
            def __getattr__(self, fname):
                if fname == 'make_decorator':
                    ...
                    raise False or AssertionError('Unknown attrname %s' % fname)
                actions.append('%s%d' % (opname, self.index))
                return res

        c1, c2, c3 = map(NameLookupTracer, [1, 2, 3])
        expected_actions = ['evalname1',
         'evalargs1',
         'makedec1',
         'evalname2',
         'evalargs2',
