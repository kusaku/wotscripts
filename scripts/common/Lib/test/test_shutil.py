# Embedded file name: scripts/common/Lib/test/test_shutil.py
import unittest
import shutil
import tempfile
import sys
...
from shutil import _make_tarball, _make_zipfile, make_archive, register_archive_format, unregister_archive_format, get_archive_formats
import tarfile
import warnings
...
class TestShutil(unittest.TestCase):
    ...
    if hasattr(os, 'chmod') and sys.platform[:6] != 'cygwin' and not (hasattr(os, 'geteuid') and os.geteuid() == 0):
    ...
    def test_copytree_simple(self):
        ...
        try:
            ...
            for path in (os.path.join(src_dir, 'test.txt'),
    ...
    def test_copytree_with_exclude(self):
        ...
        try:
            ...
            try:
                ...
                self.assertTrue(not exists(join(dst_dir, 'test_dir2', 'subdir2', 'test.py')))
                self.assertTrue(not exists(join(dst_dir, 'test_dir2', 'subdir')))
            finally:
                if os.path.exists(dst_dir):
                    shutil.rmtree(dst_dir)
...
class TestMove(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            self.dir_other_fs = tempfile.mkdtemp(dir=os.path.dirname(__file__))
            self.file_other_fs = os.path.join(self.dir_other_fs, filename)
        except OSError:
    ...
    def test_move_file_other_fs(self):
        if not self.dir_other_fs:
            return
        self._check_move_file(self.src_file, self.file_other_fs, self.file_other_fs)

    def test_move_file_to_dir_other_fs(self):
        if not self.dir_other_fs:
            return
        self._check_move_file(self.src_file, self.dir_other_fs, self.file_other_fs)
    ...
    def test_move_dir_to_dir_other_fs(self):
        if not self.dir_other_fs:
            return
        self._check_move_dir(self.src_dir, self.dir_other_fs, os.path.join(self.dir_other_fs, os.path.basename(self.src_dir)))
