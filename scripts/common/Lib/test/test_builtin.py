# Embedded file name: scripts/common/Lib/test/test_builtin.py
import platform
import unittest
from test.test_support import fcmp, have_unicode, TESTFN, unlink, run_unittest, check_py3k_warnings
import warnings
...
class BuiltinTest(unittest.TestCase):
    ...
    f.write('z = z*2\n')
    f.close()
    with check_py3k_warnings(('execfile.. not supported in 3.x', DeprecationWarning)):
        execfile(TESTFN)
    ...
    def test_filter(self):
        ...
        self.assertEqual(filter(None, [1,
         'hello',
         [],
         [3],
         '',
         ...
         [3],
         9])
        self.assertEqual(filter(lambda x: x > 0, [1,
         -3,
         9,
         0,
         2]), [1, 9, 2])
        self.assertEqual(filter(None, Squares(10)), [1,
         4,
         9,
         16,
         25,
         ...
         64,
         81])
        self.assertEqual(filter(lambda x: x % 2, Squares(10)), [1,
         9,
         25,
         49,
         81])
    ...
    def test_filter_subclasses(self):
        ...
        if have_unicode:
            ...
            inputs[unicode2] = {unicode(): unicode(),
    ...
    def test_hash(self):
        ...
        class X:
            ...
            def __hash__(self):
                return 1267650600228229401496703205376L
    ...
    def test_map(self):
        self.assertEqual(map(None, 'hello world'), ['h',
         'e',
         'l',
         'l',
         'o',
         ...
         'l',
         'd'])
        self.assertEqual(map(None, 'abcd', 'efg'), [('a', 'e'),
         ('b', 'f'),
         ('c', 'g'),
         ('d', None)])
        self.assertEqual(map(None, range(10)), [0,
         1,
         2,
         3,
         4,
        ...
        self.assertEqual(map(plus, [1, 3, 7], [4, 9, 2]), [5, 12, 9])
        self.assertEqual(map(plus, [1, 3, 7], [4, 9, 2], [1, 1, 0]), [6, 13, 9])
        self.assertEqual(map(None, Squares(10)), [0,
         1,
         4,
         9,
         16,
         ...
         64,
         81])
        self.assertEqual(map(int, Squares(10)), [0,
         1,
         4,
         9,
         16,
    ...
    def test_pow(self):
        ...
        self.assertEqual(pow(2, 20), 1048576)
        self.assertEqual(pow(2, 30), 1073741824)
        self.assertEqual(pow(-2, 0), 1)
        ...
        self.assertEqual(pow(2L, 20), 1048576)
        self.assertEqual(pow(2L, 30), 1073741824)
        self.assertEqual(pow(-2L, 0), 1)
        ...
        self.assertAlmostEqual(pow(2.0, 20), 1048576.0)
        self.assertAlmostEqual(pow(2.0, 30), 1073741824.0)
        self.assertAlmostEqual(pow(-2.0, 0), 1.0)
    ...
    def test_range(self):
        ...
        self.assertEqual(range(5, -5, -3), [5,
         2,
         -1,
         -4])
        self.assertEqual(range(-1267650600228229401496703205376L), [])
        self.assertEqual(range(0, -1267650600228229401496703205376L), [])
        self.assertEqual(range(0, 1267650600228229401496703205376L, -1), [])
        self.assertEqual(range(0, 1267650600228229401496703205376L, -1), [])
    ...
    def test_reduce(self):
        ...
        self.assertEqual(reduce(add, [['a', 'c'], [], ['d', 'w']], []), ['a',
         'c',
         'd',
         'w'])
    ...
    def test_round(self):
        ...
        self.assertEqual(type(round(0.0)), float)
        ...
        self.assertEqual(round(-8), -8)
        self.assertEqual(type(round(0)), float)
    ...
    system_round_bug = round(5000000000000001.0) != 5000000000000001.0
    ...
    def test_round_large(self):
        self.assertEqual(round(4999999999999999.0), 4999999999999999.0)
        self.assertEqual(round(5000000000000000.0), 5000000000000000.0)
        self.assertEqual(round(5000000000000001.0), 5000000000000001.0)
        self.assertEqual(round(5000000000000002.0), 5000000000000002.0)
        self.assertEqual(round(5000000000000003.0), 5000000000000003.0)
    ...
    def test_unichr(self):
        if have_unicode:
            ...
            self.assertRaises((OverflowError, ValueError), unichr, 4294967296L)
    ...
    def test_vars(self):
        ...
        self.assertEqual(self.get_vars_f2(), {'a': 1,
         'b': 2})
    ...
    def test_zip(self):
        ...
        self.assertEqual(zip(SequenceWithoutALength(), xrange(1073741824)), list(enumerate(range(5))))
    ...
    def test_format(self):
        ...
        empty_format_spec(9904578032905937L)
        ...
        def test_deprecated_format_string(obj, fmt_str, should_raise_warning):
            ...
            if should_raise_warning:
                ...
                self.assertIn('object.__format__ with a non-empty format string', str(w[0].message))
            else:
    ...
    def test_bin(self):
        ...
        self.assertEqual(bin(-1), '-0b1')
        self.assertEqual(bin(36893488147419103232L), '0b1' + '0' * 65)
        self.assertEqual(bin(36893488147419103231L), '0b' + '1' * 65)
        self.assertEqual(bin(-36893488147419103232L), '-0b1' + '0' * 65)
        self.assertEqual(bin(-36893488147419103231L), '-0b' + '1' * 65)
...
def _run_unittest(*args):
    with check_py3k_warnings(('.+ not supported in 3.x', DeprecationWarning), ('.+ is renamed to imp.reload', DeprecationWarning), ('classic int division', DeprecationWarning)):
        run_unittest(*args)
