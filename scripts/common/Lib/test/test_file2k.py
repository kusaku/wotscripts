# Embedded file name: scripts/common/Lib/test/test_file2k.py
import sys
import os
import unittest
import itertools
...
class OtherFileTests(unittest.TestCase):
    ...
    def testStdin(self):
        if sys.platform != 'osf1V5':
            ...
            print >> sys.__stdout__, '  Skipping sys.stdin.seek(-1), it may crash the interpreter. Test manually.'
...
class FileThreadingTests(unittest.TestCase):
    ...
    def _create_file(self):
        if self.use_buffering:
            self.f = open(self.filename, 'w+', buffering=16384)
        else:
    ...
    def _test_close_open_io(self, io_func, nb_workers = 5):
        ...
        def worker():
            self._create_file()
            funcs = itertools.cycle((lambda : io_func(), lambda : self._close_and_reopen_file()))
            for f in funcs:
        ...
        if test_support.verbose:
            percent = 100.0 - 100.0 * self.close_success_count / self.close_count
...
class StdoutTests(unittest.TestCase):
    ...
    def test_unicode(self):
        ...
        def get_message(encoding, *code):
            ...
            process = subprocess.Popen([sys.executable, '-c', code], stdout=subprocess.PIPE, env=env)
            stdout, stderr = process.communicate()
            self.assertEqual(process.returncode, 0)
            return stdout

        def check_message(text, encoding, expected):
            stdout = get_message(encoding, 'import sys', 'sys.stdout.write(%r)' % text, 'sys.stdout.flush()')
            self.assertEqual(stdout, expected)

        check_message(u'15\u20ac', 'iso-8859-15', '15\xa4')
        check_message(u'15\u20ac', 'utf-8', '15\xe2\x82\xac')
        check_message(u'15\u20ac', 'utf-16-le', '1\x005\x00\xac ')
        ...
        for objtype in ('buffer', 'bytearray'):
            stdout = get_message('ascii', 'import sys', 'sys.stdout.write(%s("\\xe9"))' % objtype, 'sys.stdout.flush()')
            self.assertEqual(stdout, '\xe9')


def test_main():
    try:
        run_unittest(AutoFileTests, OtherFileTests, FileSubclassTests, FileThreadingTests, StdoutTests)
    finally:
