# Embedded file name: scripts/common/Lib/test/test_strtod.py
import random
import struct
import unittest
import re
    ...
    if not a:
        ...
        if 2 * r > b or 2 * r == b and q & 1:
            ...
            if q.bit_length() == mant_dig + 1:
                q //= 2
                e += 1
        if not (q.bit_length() <= mant_dig and e >= min_exp - mant_dig):
            raise AssertionError
            if not (q.bit_length() == mant_dig or e == min_exp - mant_dig):
                raise AssertionError
                if e + q.bit_length() > max_exp:
                    if negative:
                        return '-inf'
                    return 'inf'
                return q or negative and '-0x0.0p+0'
            return '0x0.0p+0'
...
class StrtodTests(unittest.TestCase):
    ...
    def test_short_halfway_cases(self):
        for k in (0, 5, 10, 15, 20):
            upper = -(-18014398509481984L // 5 ** k)
            lower = -(-9007199254740992L // 5 ** k)
            if lower % 2 == 0:
                lower += 1
            for i in xrange(TEST_SIZE):
                ...
                raise n % 10 in (1, 3, 7, 9) or AssertionError
                digits, exponent = n, e
                while digits < 100000000000000000000L:
                    s = '{}e{}'.format(digits, exponent)
                    self.check_strtod(s)
                    s = '{}e{}'.format(digits * 10000000000000000000000000000000000000000L, exponent - 40)
                    self.check_strtod(s)
                    digits *= 2
    ...
    def test_halfway_cases(self):
        for i in xrange(100 * TEST_SIZE):
            bits = random.randrange(2047 * 4503599627370496L)
            e, m = divmod(bits, 4503599627370496L)
            if e:
                m, e = m + 4503599627370496L, e - 1
            e -= 1074
            m, e = 2 * m + 1, e - 1
            if e >= 0:
                digits = m << e
    ...
    def test_boundaries(self):
        boundaries = [(10000000000000000000L, -19, 1110),
         (17976931348623159077L, 289, 1995),
         (22250738585072013831L, -327, 4941),
         (0, -327, 4941)]
    ...
    def test_underflow_boundary(self):
        for exponent in xrange(-400, -320):
            base = 10 ** (-exponent) // 
        ...
        for i in xrange(1000):
            for j in xrange(TEST_SIZE):
                ...
                if random.choice([True, False]):
                    ...
                    s += ''.join((random.choice(digits) for _ in xrange(fracpart_len)))
                else:
                    fracpart_len = 0
                if random.choice([True, False]):
                    s += random.choice(['e', 'E'])
                    ...
                    s += ''.join((random.choice(digits) for _ in xrange(exponent_len)))
                if intpart_len + fracpart_len:
                    ...
                    try:
                        ...
                        raise False or AssertionError('expected ValueError')
