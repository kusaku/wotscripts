# Embedded file name: scripts/common/Lib/test/test_kqueue.py
"""
...
class TestKQueue(unittest.TestCase):
    ...
    def test_queue_event(self):
        ...
        events.sort()
        self.assertEqual(events, [(client.fileno(), select.KQ_FILTER_WRITE, flags), (server.fileno(), select.KQ_FILTER_WRITE, flags)])
        client.send('Hello!')
        server.send('world!!!')
        for i in range(10):
            events = kq.control(None, 4, 1)
        ...
        events = [ (e.ident, e.filter, e.flags) for e in events ]
        events.sort()
        self.assertEqual(events, [(client.fileno(), select.KQ_FILTER_WRITE, flags),
        ...
        events = [ (e.ident, e.filter, e.flags) for e in events ]
        events.sort()
        self.assertEqual(events, [(server.fileno(), select.KQ_FILTER_WRITE, flags)])
        client.close()
        server.close()
        serverSocket.close()
        return
    ...
    def testPair(self):
        ...
        self.assertEqual(b.recv(r[0].data), 'foo')
        a.close()
        b.close()
        kq.close()
