# Embedded file name: scripts/common/Lib/test/test_py3kwarn.py
import unittest
import sys
...
try:
    ...
    def check_deprecated_module(module_name):
        ...
        """
        return any((module_name in msg and ' removed' in msg and issubclass(cls, DeprecationWarning) and (' module' in msg or ' package' in msg) for msg, cls, line in past_warnings))
...
class TestPy3KWarnings(unittest.TestCase):
    ...
    def test_forbidden_names(self):
        ...
        tests = [('True', 'assignment to True or False is forbidden in 3.x'), ('False', 'assignment to True or False is forbidden in 3.x'), ('nonlocal', 'nonlocal is a keyword in 3.x')]
    ...
    def test_builtin_function_or_method_comparisons(self):
        expected = 'builtin_function_or_method order comparisons not supported in 3.x'
        func = eval
        meth = {}.get
        with check_py3k_warnings() as w:
    ...
    def test_slice_methods(self):
        ...
        class Spam(object):
            ...
            def __setslice__(self, i, j, what):
                pass
    ...
    def test_file_xreadlines(self):
        expected = "f.xreadlines() not supported in 3.x, try 'for line in f' instead"
        with file(__file__) as f:
            with check_py3k_warnings() as w:
    ...
    def test_hash_inheritance(self):
        with check_py3k_warnings() as w:
            ...
            self.assertWarning(None, w, 'Overriding __eq__ blocks inheritance of __hash__ in 3.x')
            w.reset()
    ...
    def test_operator(self):
        ...
        seq_warn = 'operator.sequenceIncludes() is not supported in 3.x. Use operator.contains().'
        with check_py3k_warnings() as w:
...
class TestStdlibRemovals(unittest.TestCase):
    all_platforms = ('audiodev', 'imputil', 'mutex', 'user', 'new', 'rexec', 'Bastion', 'compiler', 'dircache', 'mimetools', 'fpformat', 'ihooks', 'mhlib', 'statvfs', 'htmllib', 'sgmllib', 'rfc822', 'sunaudio')
    inclusive_platforms = {'irix': ('pure', 'AL', 'al', 'CD', 'cd', 'cddb', 'cdplayer', 'CL', 'cl', 'DEVICE', 'GL', 'gl', 'ERRNO', 'FILE', 'FL', 'flp', 'fl', 'fm', 'GET', 'GLWS', 'imgfile', 'IN', 'IOCTL', 'jpeg', 'panel', 'panelparser', 'readcd', 'SV', 'torgb', 'WAIT'),
     'darwin': ('autoGIL', 'Carbon', 'OSATerminology', 'icglue', 'Nav', 'aepack', 'aetools', 'aetypes', 'applesingle', 'appletrawmain', 'appletrunner', 'argvemulator', 'bgenlocations', 'EasyDialogs', 'macerrors', 'macostools', 'findertools', 'FrameWork', 'ic', 'gensuitemodule', 'icopen', 'macresource', 'MiniAEFrame', 'pimp', 'PixMapWrapper', 'terminalcommand', 'videoreader', '_builtinSuites', 'CodeWarrior', 'Explorer', 'Finder', 'Netscape', 'StdSuites', 'SystemEvents', 'Terminal', 'cfmfile', 'bundlebuilder', 'buildtools', 'ColorPicker', 'Audio_mac'),
     'sunos5': ('sunaudiodev', 'SUNAUDIODEV')}
    optional_modules = ('bsddb185', 'Canvas', 'dl', 'linuxaudiodev', 'imageop', 'sv', 'bsddb', 'dbhash')
    ...
    def test_mutablestring_removal(self):
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings('error', '.*MutableString', DeprecationWarning)
