# Embedded file name: scripts/common/Lib/test/test_math.py
...
def acc_check(expected, got, rel_err = 2e-15, abs_err = 5e-323):
    ...
    if abs(error) < permitted_error:
        ...
        return 'error = {}; permitted error = {}'.format(error, permitted_error)
...
class MathTests(unittest.TestCase):
    ...
    def testCosh(self):
        ...
        self.ftest('cosh(2)-2*cosh(1)**2', math.cosh(2) - 2 * math.cosh(1) ** 2, -1)
    ...
    def testFrexp(self):
        ...
        def testfrexp(name, result, expected):
            ...
            if abs(mant - emant) > eps or exp != eexp:
                self.fail('%s returned %r, expected %r' % (name, (mant, exp), (emant, eexp)))
    ...
    @requires_IEEE_754
    @unittest.skipIf(HAVE_DOUBLE_ROUNDING, 'fsum is not exact on machines with double rounding')
    def testFsum(self):
        ...
        test_values = [([], 0.0),
         ...
         ([1e+100,
           ...
           -1.0,
           -1e+50], 1e-100),
         ([9007199254740992.0, -0.5, -5.551115123125783e-17], 9007199254740991.0),
         ([9007199254740992.0, 1.0, 7.888609052210118e-31], 9007199254740994.0),
         ([9007199254741002.0, 1.0, 7.888609052210118e-31], 9007199254741004.0),
         ([9007199254740988.0, 0.5, 5.551115123125783e-17], 9007199254740989.0),
         ([ 1.0 / n for n in range(1, 1001) ], float.fromhex('0x1.df11f45f4e61ap+2')),
         ([ (-1.0) ** n / n for n in range(1, 1001) ], float.fromhex('-0x1.62a2af1bd3624p-1')),
         ([ 1.7 ** (i + 1) - 1.7 ** i for i in range(1000) ] + [-2.8113918290273277e+230], -1.0),
         ([1e+16, 1.0, 1e-16], 1.0000000000000002e+16),
         ([9999999999999998.0,
           1.0 - 1.1102230246251565e-16,
           -9999999999999998.0,
           -(1.0 - 1.1102230246251565e-16)], 0.0),
         ([ 2.0 ** n - 2.0 ** (n + 50) + 2.0 ** (n + 52) for n in range(-1074, 972, 2) ] + [-4.49423283715579e+307], float.fromhex('0x1.5555555555555p+970'))]
        for i, (vals, expected) in enumerate(test_values):
            try:
                actual = math.fsum(vals)
            except OverflowError:
                self.fail('test %d failed: got OverflowError, expected %r for math.fsum(%.100r)' % (i, expected, vals))
            except ValueError:
                self.fail('test %d failed: got ValueError, expected %r for math.fsum(%.100r)' % (i, expected, vals))
    ...
    def testLog(self):
        ...
        self.ftest('log(10**40, 10)', math.log(10000000000000000000000000000000000000000L, 10), 40)
        self.ftest('log(10**40, 10**20)', math.log(10000000000000000000000000000000000000000L, 100000000000000000000L), 2)
    ...
    def testLog1p(self):
        ...
        n = 1237940039285380274899124224L
    ...
    def testModf(self):
        ...
        def testmodf(name, result, expected):
            ...
            if abs(v1 - e1) > eps or abs(v2 - e2):
                self.fail('%s returned %r, expected %r' % (name, (v1, v2), (e1, e2)))
    ...
    def test_trunc(self):
        ...
        self.assertRaises((AttributeError, TypeError), math.trunc, TestNoTrunc())
        ...
        for id, fn, arg, expected, flags in parse_mtestfile(math_testcases):
            ...
            if isinstance(got, float) and isinstance(expected, float):
                ...
                if not math.isnan(expected) and not math.isnan(got):
                    if fn == 'lgamma':
                        accuracy_failure = acc_check(expected, got, rel_err=5e-15, abs_err=5e-15)
                    elif fn == 'erfc':
