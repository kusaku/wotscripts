# Embedded file name: scripts/common/Lib/test/test_tarfile.py
import sys
import os
import shutil
import StringIO
...
class UstarReadTest(ReadTest):
    ...
    def test_fileobj_regular_file(self):
        ...
        data = fobj.read()
        self.assertTrue((len(data), md5sum(data)) == (tarinfo.size, md5_regtype), 'regular file extraction failed')
    ...
    def test_fileobj_readlines(self):
        ...
        self.assertTrue(lines1 == lines2, 'fileobj.readlines() failed')
        self.assertTrue(len(lines2) == 114, 'fileobj.readlines() failed')
    ...
    def test_fileobj_iter(self):
        ...
        self.assertTrue(lines1 == lines2, 'fileobj.__iter__() failed')
    ...
    def test_fileobj_seek(self):
        ...
        text = fobj.read()
        fobj.seek(0)
        self.assertTrue(0 == fobj.tell(), "seek() to file's start failed")
        fobj.seek(2048, 0)
        self.assertTrue(2048 == fobj.tell(), 'seek() to absolute position failed')
        fobj.seek(-1024, 1)
        self.assertTrue(1024 == fobj.tell(), 'seek() to negative relative position failed')
        fobj.seek(1024, 1)
        self.assertTrue(2048 == fobj.tell(), 'seek() to positive relative position failed')
        s = fobj.read(10)
        self.assertTrue(s == data[2048:2058], 'read() after seek failed')
        fobj.seek(0, 2)
        self.assertTrue(tarinfo.size == fobj.tell(), "seek() to file's end failed")
        self.assertTrue(fobj.read() == '', "read() at file's end did not return empty string")
        fobj.seek(-tarinfo.size, 2)
        self.assertTrue(0 == fobj.tell(), "relative seek() to file's start failed")
        fobj.seek(512)
        s1 = fobj.readlines()
        fobj.seek(512)
        s2 = fobj.readlines()
        self.assertTrue(s1 == s2, 'readlines() after seek failed')
        fobj.seek(0)
        self.assertTrue(len(fobj.readline()) == fobj.tell(), 'tell() after readline() failed')
        fobj.seek(512)
        self.assertTrue(len(fobj.readline()) + 512 == fobj.tell(), 'tell() after seek() and readline() failed')
        fobj.seek(0)
        line = fobj.readline()
        self.assertTrue(fobj.read() == data[len(line):], 'read() after readline() failed')
        fobj.close()
...
class MiscReadTest(CommonReadTest):
    ...
    def test_v7_dirtype(self):
        ...
        self.assertTrue(tarinfo.type == tarfile.DIRTYPE, 'v7 dirtype failed')
    ...
    def test_check_members(self):
        for tarinfo in self.tar:
            self.assertTrue(int(tarinfo.mtime) == 1041808783, 'wrong mtime for %s' % tarinfo.name)
            if not tarinfo.name.startswith('ustar/'):
                continue
            self.assertTrue(tarinfo.uname == 'tarfile', 'wrong uname for %s' % tarinfo.name)

    def test_find_members(self):
        self.assertTrue(self.tar.getmembers()[-1].name == 'misc/eof', 'could not find all members')
...
class StreamReadTest(CommonReadTest):
    ...
    def test_provoke_stream_error(self):
        ...
        f = self.tar.extractfile(tarinfos[0])
...
class MemberReadTest(ReadTest):
    ...
    def _test_member(self, tarinfo, chksum = None, **kwargs):
        if chksum is not None:
            self.assertTrue(md5sum(self.tar.extractfile(tarinfo).read()) == chksum, 'wrong md5sum for %s' % tarinfo.name)
        ...
        for k, v in kwargs.iteritems():
            self.assertTrue(getattr(tarinfo, k) == v, 'wrong value in %s field of %s' % (k, tarinfo.name))
...
class GNUReadTest(LongnameTest):
    ...
    def test_sparse_file(self):
        ...
        self.assertTrue(fobj1.read() == fobj2.read(), 'sparse file extraction failed')
...
class WriteTest(WriteTestBase):
    ...
    def test_100_char_name(self):
        ...
        self.assertTrue(tar.getnames()[0] == name, 'failed to store 100 char filename')
        tar.close()
    ...
    def test_tar_size(self):
        ...
        tar.add(path)
        tar.close()
        self.assertTrue(os.path.getsize(tmpname) > 0, 'tarfile is empty')
    ...
    def test_exclude(self):
        ...
        try:
            ...
            with test_support.check_warnings(('use the filter argument', DeprecationWarning)):
...
class StreamWriteTest(WriteTestBase):
    ...
    def test_stream_padding(self):
        ...
        if self.mode.endswith('gz'):
            ...
            self.assertTrue(len(dec.unused_data) == 0, 'found trailing data')
        else:
            fobj = open(tmpname, 'rb')
            data = fobj.read()
            fobj.close()
        self.assertTrue(data.count('\x00') == tarfile.RECORDSIZE, 'incorrect zero padding')
...
class HardlinkTest(unittest.TestCase):
    ...
    def test_add_twice(self):
        ...
        self.assertTrue(tarinfo.type == tarfile.REGTYPE, 'add file as regular failed')

    def test_add_hardlink(self):
        tarinfo = self.tar.gettarinfo(self.bar)
        self.assertTrue(tarinfo.type == tarfile.LNKTYPE, 'add file as hardlink failed')

    def test_dereference_hardlink(self):
        self.tar.dereference = True
        tarinfo = self.tar.gettarinfo(self.bar)
        self.assertTrue(tarinfo.type == tarfile.REGTYPE, 'dereferencing hardlink failed')
...
class PaxWriteTest(GNUWriteTest):
    ...
    def test_pax_extended_header(self):
        pax_headers = {u'path': u'foo',
         u'uid': u'123'}
        tar = tarfile.open(tmpname, 'w', format=tarfile.PAX_FORMAT, encoding='iso8859-1')
        t = tarfile.TarInfo()
        t.name = u'\xe4\xf6\xfc'
        t.uid = 16777216
...
class PaxUnicodeTest(UstarUnicodeTest):
    ...
    def test_error_handlers(self):
        ...
        for handler, name in (('utf-8', u'\xe4\xf6\xfc'.encode('utf8')), ('replace', '???'), ('ignore', '')):
            tar = tarfile.open(tmpname, format=self.format, encoding='ascii', errors=handler)
            self.assertEqual(tar.getnames()[0], name)

        self.assertRaises(UnicodeError, tarfile.open, tmpname, encoding='ascii', errors='strict')

    def test_error_handler_utf8(self):
        self._create_unicode_name(u'\xe4\xf6\xfc/\u20ac')
        tar = tarfile.open(tmpname, format=self.format, encoding='iso8859-1', errors='utf-8')
...
class AppendTest(unittest.TestCase):
    ...
    def test_empty_fileobj(self):
        fobj = StringIO.StringIO('\x00' * 1024)
    ...
    def test_incomplete(self):
        self._test_error('\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00')
    ...
    def test_trailing_garbage(self):
        data = tarfile.TarInfo('foo').tobuf()
        self._test_error(data + '\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00')
...
class LimitsTest(unittest.TestCase):
    ...
    def test_gnu_limits(self):
        ...
        tarinfo.tobuf(tarfile.GNU_FORMAT)
        tarinfo = tarfile.TarInfo('name')
        tarinfo.uid = 72057594037927936L
...
class ContextManagerTest(unittest.TestCase):
    ...
    def test_exception(self):
        with self.assertRaises(Exception) as exc:
            with tarfile.open(tarname) as tar:
                raise IOError
        self.assertIsInstance(exc.exception, IOError, 'wrong exception raised in context manager')
        self.assertTrue(tar.closed, 'context manager failed')
