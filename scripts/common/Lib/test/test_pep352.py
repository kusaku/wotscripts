# Embedded file name: scripts/common/Lib/test/test_pep352.py
import unittest
import __builtin__
import exceptions
import warnings
...
if sys.py3kwarning:
    DEPRECATION_WARNINGS.extend(['exceptions must derive from BaseException', "catching classes that don't inherit from BaseException is not allowed", '__get(item|slice)__ not supported for exception classes'])
...
class ExceptionClassTests(unittest.TestCase):
    ...
    def verify_instance_interface(self, ins):
        for attr in ('args', 'message', '__str__', '__repr__', '__getitem__'):
            self.assertTrue(hasattr(ins, attr), '%s missing %s attribute' % (ins.__class__.__name__, attr))
    ...
    interface_tests = ('length', 'args', 'message', 'str', 'unicode', 'repr', 'indexing')

    def interface_test_driver(self, results):
        for test_name, (given, expected) in zip(self.interface_tests, results):
            self.assertEqual(given, expected, '%s: %s != %s' % (test_name, given, expected))

    @ignore_deprecation_warnings
    def test_interface_single_arg(self):
        arg = 'spam'
        exc = Exception(arg)
        results = ([len(exc.args), 1],
         [exc.args[0], arg],
         [exc.message, arg],
         [str(exc), str(arg)],
    ...
    def test_interface_multi_arg(self):
        ...
        exc = Exception(*args)
        results = ([len(exc.args), arg_count],
         [exc.args, args],
         [exc.message, ''],
         [str(exc), str(args)],
...
class UsageTests(unittest.TestCase):
    ...
    def catch_fails(self, object_):
        ...
        try:
            ...
            self.fail('TypeError expected when catching %s as specified in a tuple' % type(object_))
