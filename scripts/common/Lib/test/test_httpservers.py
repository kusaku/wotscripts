# Embedded file name: scripts/common/Lib/test/test_httpservers.py
...
class BaseHTTPRequestHandlerTestCase(unittest.TestCase):
    ...
    def test_request_length(self):
        result = self.send_typical_request('GET ' + 'x' * 65537)
        self.assertEqual(result[0], 'HTTP/1.1 414 Request-URI Too Long\r\n')
...
class SimpleHTTPServerTestCase(BaseTestCase):
    ...
    def test_head(self):
        response = self.request(self.tempdir_name + '/test', method='HEAD')
        self.check_status_and_reason(response, 200)
...
class CGIHTTPServerTestCase(BaseTestCase):
    ...
    def test_authorization(self):
        headers = {'Authorization': 'Basic %s' % base64.b64encode('username:pass')}
...
def test_main(verbose = None):
    try:
        cwd = os.getcwd()
        test_support.run_unittest(BaseHTTPRequestHandlerTestCase, SimpleHTTPRequestHandlerTestCase, BaseHTTPServerTestCase, SimpleHTTPServerTestCase, CGIHTTPServerTestCase)
    finally:
        os.chdir(cwd)
