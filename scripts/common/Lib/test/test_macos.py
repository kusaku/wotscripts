# Embedded file name: scripts/common/Lib/test/test_macos.py
import unittest
...
class TestMacOS(unittest.TestCase):
    ...
    def testGetCreatorAndType(self):
        ...
        try:
            ...
            fp.write('\n')
            fp.close()
            subprocess.call(['/Developer/Tools/SetFile',
             '-t',
             'ABCD',
             '-c',
             'EFGH',
    ...
    def testSetCreatorAndType(self):
        if not os.path.exists('/Developer/Tools/GetFileInfo'):
            ...
            try:
                ...
                data = subprocess.Popen(['/Developer/Tools/GetFileInfo', test_support.TESTFN], stdout=subprocess.PIPE).communicate()[0]
                tp = None
                cr = None
