# Embedded file name: scripts/common/Lib/test/test_str.py
import struct
import sys
from test import test_support, string_tests

class StrTest(string_tests.CommonTest, string_tests.MixinStrUnicodeUserStringTest, string_tests.MixinStrUserStringTest, string_tests.MixinStrUnicodeTest):
    type2test = str
    ...
    def test_formatting(self):
        ...
        self.assertRaises(OverflowError, '%c'.__mod__, 4660)
    ...
    def test_buffer_is_readonly(self):
        self.assertRaises(TypeError, sys.stdin.readinto, '')

    def test_encode_and_decode_kwargs(self):
        self.assertEqual('abcde'.encode('ascii', 'replace'), 'abcde'.encode('ascii', errors='replace'))
        self.assertEqual('abcde'.encode('ascii', 'ignore'), 'abcde'.encode(encoding='ascii', errors='ignore'))
        self.assertEqual('Andr\x82 x'.decode('ascii', 'ignore'), 'Andr\x82 x'.decode('ascii', errors='ignore'))
        self.assertEqual('Andr\x82 x'.decode('ascii', 'replace'), 'Andr\x82 x'.decode(encoding='ascii', errors='replace'))
