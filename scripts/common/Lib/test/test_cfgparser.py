# Embedded file name: scripts/common/Lib/test/test_cfgparser.py
import ConfigParser
import StringIO
import os
import unittest
...
class TestCaseBase(unittest.TestCase):
    ...
    def newconfig(self, defaults = None):
        if defaults is None:
            ...
            self.cf = self.config_class(defaults, allow_no_value=self.allow_no_value)
        return self.cf
    ...
    def test_basic(self):
        ...
        self.assertNotIn('__name__', cf.options('Foo Bar'), '__name__ "option" should not be exposed by the API!')
    ...
    def test_default_case_sensitivity(self):
        ...
        self.assertEqual(cf.get('DEFAULT', 'Foo'), 'Bar', 'could not locate option, expecting case-insensitive option names')
        cf = self.newconfig({'Foo': 'Bar'})
        self.assertEqual(cf.get('DEFAULT', 'Foo'), 'Bar', 'could not locate option, expecting case-insensitive defaults')
    ...
    def test_query_errors(self):
        ...
        self.assertRaises(ConfigParser.NoSectionError, cf.options, 'Foo')
        self.assertRaises(ConfigParser.NoSectionError, cf.set, 'foo', 'bar', 'value')
    ...
    def get_error(self, exc, section, option):
        ...
        self.fail('expected exception type %s.%s' % (exc.__module__, exc.__name__))
    ...
    def test_weird_errors(self):
        cf = self.newconfig()
        cf.add_section('Foo')
        self.assertRaises(ConfigParser.DuplicateSectionError, cf.add_section, 'Foo')
    ...
    def check_items_config(self, expected):
        cf = self.fromstring('[section]\nname = value\nkey: |%(name)s| \ngetdefault: |%(default)s|\ngetname: |%(__name__)s|', defaults={'default': '<default>'})
...
class ConfigParserTestCase(TestCaseBase):
    ...
    def test_interpolation(self):
        rawval = {ConfigParser.ConfigParser: 'something %(with11)s lots of interpolation (11 steps)',
    ...
    def test_interpolation_missing_value(self):
        ...
        e = self.get_error(ConfigParser.InterpolationError, 'Interpolation Error', 'name')
    ...
    def test_set_nonstring_types(self):
        ...
        self.assertEqual(cf.get('non-string', 'list', raw=True), [0,
         1,
         1,
         2,
         3,
        ...
        self.assertEqual(cf.get('non-string', 'dict', raw=True), {'pi': 3.14159,
         '%(': 1,
         '%(list)': '%(list)'})
        self.assertRaises(TypeError, cf.get, 'non-string', 'dict')
        self.assertEqual(cf.get('non-string', 'string_with_interpolation', raw=True), '%(list)s')
        self.assertRaises(ValueError, cf.get, 'non-string', 'string_with_interpolation', raw=False)
...
class RawConfigParserTestCase(TestCaseBase):
    ...
    def test_set_nonstring_types(self):
        ...
        self.assertEqual(cf.get('non-string', 'list'), [0,
         1,
         1,
         2,
         3,
...
class TestChainMap(unittest.TestCase):
    ...
    def test_issue_12717(self):
        ...
        self.assertEqual(set(cm.keys()), set(dcomb.keys()))
        self.assertEqual(set(cm.values()), set(dcomb.values()))
        self.assertEqual(set(cm.items()), set(dcomb.items()))
        self.assertEqual(set(cm), set(dcomb))
        self.assertEqual(cm, dcomb)
        self.assertEqual([ cm[k] for k in dcomb ], dcomb.values())
        klist = 'red green blue black brown'.split()
        self.assertEqual([ cm.get(k, 10) for k in klist ], [ dcomb.get(k, 10) for k in klist ])
        self.assertEqual([ k in cm for k in klist ], [ k in dcomb for k in klist ])
        with test_support.check_py3k_warnings():
            self.assertEqual([ cm.has_key(k) for k in klist ], [ dcomb.has_key(k) for k in klist ])
...
def test_main():
    test_support.run_unittest(ConfigParserTestCase, MultilineValuesTestCase, RawConfigParserTestCase, SafeConfigParserTestCase, SafeConfigParserTestCaseNoValue, SortedTestCase, Issue7005TestCase, TestChainMap, ExceptionPicklingTestCase)
