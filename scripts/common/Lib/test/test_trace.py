# Embedded file name: scripts/common/Lib/test/test_trace.py
import os
import sys
from test.test_support import run_unittest, TESTFN, rmtree, unlink, captured_stdout
import unittest
import trace
...
class TestLineCounts(unittest.TestCase):
    ...
    def test_trace_func_generator(self):
        ...
        expected = {(self.my_py_filename, firstlineno_calling + 1): 1,
...
class TestFuncs(unittest.TestCase):
    ...
    def test_simple_caller(self):
        ...
        expected = {self.filemod + ('traced_func_simple_caller',): 1,
    ...
    def test_inst_method_calling(self):
        ...
        expected = {self.filemod + ('TracedClass.inst_method_calling',): 1,
...
class TestCallers(unittest.TestCase):
    ...
    def test_loop_caller_importing(self):
        self.tracer.runfunc(traced_func_importing_caller, 1)
        expected = {((os.path.splitext(trace.__file__)[0] + '.py', 'trace', 'Trace.runfunc'), self.filemod + ('traced_func_importing_caller',)): 1,
         (self.filemod + ('traced_func_simple_caller',), self.filemod + ('traced_func_linear',)): 1,
         (self.filemod + ('traced_func_importing_caller',), self.filemod + ('traced_func_simple_caller',)): 1,
         (self.filemod + ('traced_func_importing_caller',), self.filemod + ('traced_func_importing',)): 1,
         (self.filemod + ('traced_func_importing',), (fix_ext_py(testmod.__file__), 'testmod', 'func')): 1}
...
class TestCoverage(unittest.TestCase):
    ...
    def _coverage(self, tracer, cmd = 'from test import test_pprint; test_pprint.test_main()'):
        tracer.run(cmd)
        r = tracer.results()
    ...
    def test_coverage(self):
        ...
        self.assertTrue('case.py' in stdout)
    ...
    def test_coverage_ignore(self):
        ...
        tracer = trace.Trace(ignoredirs=[sys.prefix, sys.exec_prefix, libpath], trace=0, count=1)
        with captured_stdout() as stdout:
            self._coverage(tracer)
