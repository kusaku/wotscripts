# Embedded file name: scripts/common/Lib/test/test_support.py
...
def get_attribute(obj, name):
    ...
    try:
        ...
        raise unittest.SkipTest('module %s has no attribute %s' % (obj.__name__, name))
    else:
        return attribute
    ...
    if have_unicode:
        ...
        if not hasattr(sys, 'getwindowsversion') or sys.getwindowsversion()[3] < 2:
            ...
            try:
                ...
                print 'WARNING: The filename %r CAN be encoded by the filesystem.  Unicode filename tests may not be effective' % TESTFN_UNENCODABLE
...
def transient_internet(resource_name, timeout = 30.0, errnos = ()):
    ...
    default_errnos = [('ECONNREFUSED', 111),
     ('ECONNRESET', 104),
     ('EHOSTUNREACH', 113),
     ('ENETUNREACH', 101),
     ('ETIMEDOUT', 110)]
    default_gai_errnos = [('EAI_AGAIN', -3),
     ('EAI_FAIL', -4),
     ('EAI_NONAME', -2),
     ('EAI_NODATA', -5)]
...
def _parse_guards(guards):
    ...
    raise guards.values() == [is_true] * len(guards) or AssertionError
