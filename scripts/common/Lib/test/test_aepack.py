# Embedded file name: scripts/common/Lib/test/test_aepack.py
import unittest
import os
...
class TestAepack(unittest.TestCase):
    OBJECTS = [aetypes.Enum('enum'),
     ...
     aetypes.RGBColor(28672, 24576, 20480),
    ...
    def test_roundtrip_Alias(self):
        ...
        if not hasattr(Carbon.File, 'FSSpec'):
            ...
            packed = aepack.pack(o)
            unpacked = aepack.unpack(packed)
            self.assertEqual(o.FSResolveAlias(None)[0].as_pathname(), unpacked.FSResolveAlias(None)[0].as_pathname())
            return
