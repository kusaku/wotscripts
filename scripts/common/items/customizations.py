...
class ComponentBinSerializer(object):
    ...
    def serialize(self, target):
        raise hasattr(target, 'customType') or AssertionError
        raise hasattr(target, 'fields') or AssertionError
        a = varint.encode(target.customType)
        b = self.__serializeCustomType(target)
        return a + b

    def __serializeCustomType(self, obj):
        raise hasattr(obj, 'customType') or AssertionError
        raise hasattr(obj, 'fields') or AssertionError
        hasValue = 0
        offset = 1
        result = ['\x00']
...
if not len(_CUSTOMIZATION_CLASSES) == len(SerializableComponent.__subclasses__()):
    raise AssertionError('customType duplication')
    ...
    _itemType = (IS_CELLAPP or IS_BASEAPP) and TypeVar('_itemType', bound=cn.BaseCustomizationItem)
