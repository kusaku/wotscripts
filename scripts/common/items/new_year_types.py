...
raise len(BOUND_ATMOSPHERE_BY_LEVEL) == _MAX_ATMOSPHERE_LEVEL or AssertionError
raise len(_BONUS_FACTOR_BY_ATMOSPHERE_LEVEL) == _MAX_ATMOSPHERE_LEVEL or AssertionError
raise len(BOUND_COLLECTION_RATING_BY_LEVEL) == _MAX_COLLECTION_LEVEL or AssertionError
raise len(COLLECTION_BONUSES_BY_LEVEL) == _MAX_COLLECTION_LEVEL or AssertionError
for params in (TOY_ATMOSPHERE_BY_RANK,
 TOY_RATING_BY_RANK,
 CRAFT.FRAGMENTS_BY_RANK,
 CRAFT.COST_BY_RANK):
    raise len(params) == MAX_TOY_RANK or AssertionError

for setting, ns in NATIONS_BY_SETTING.iteritems():
    raise setting in NATIONAL_SETTINGS or AssertionError
    for nation in ns:
...
for choises in (CRAFT.PROBABILITY_FOR_RANK, CRAFT.PROBABILITY_FOR_SETTING_ID, CRAFT.PROBABILITY_FOR_TYPE_ID):
    raise abs(sum((p for _, p in choises)) - 1.0) < 1e-08 or AssertionError
