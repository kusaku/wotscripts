...
from consts import CREDENTIALS_RESTRICTIONS, CREDENTIALS_RESTRICTIONS_SET, IS_CLIENT
import unicodedata
if CREDENTIALS_RESTRICTIONS_SET == CREDENTIALS_RESTRICTIONS.BASIC:
...
elif CREDENTIALS_RESTRICTIONS_SET == CREDENTIALS_RESTRICTIONS.CHINESE:
...
def utf8_accepted(utf8str, re, minLen, maxLen, unicodeNormalForm = 'NFKC', checkBeforeNFKC = True):
    nfkc, plain = unicode_from_utf8(utf8str, unicodeNormalForm)
    matchFn = lambda uniStr: re.match(uniStr) and minLen <= len(uniStr) <= maxLen
    if checkBeforeNFKC and not matchFn(plain):
        return False
    return matchFn(nfkc)
...
def normalized_unicode_trim_and_lowercase(utf8str, length):
    try:
        unicodeStr, _ = unicode_from_utf8(utf8str)
        if len(unicodeStr) > max(0, length):
            unicodeStr = unicodeStr[:length]
